...
- The official Stage child plan requires `--profile-report`, a root `--authorization`, and a protected `--child-authorization`; it derives the child owner from the accepted profile and rejects caller-selected owner input. A Stage cleanup plan requires one observation for every bound resource. Missing, dirty, unknown or SHA-mismatched resources are preserved as failures. Removal succeeds only with an explicit post-removal `exists=false` observation; verified completed keys are skipped on retry.
