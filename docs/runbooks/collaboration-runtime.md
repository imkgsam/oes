...
- The official Stage child plan requires `--profile-report`, a root `--authorization`, and a protected `--child-authorization`; it also reopens the fixed profile-protected `current-stage-cleanup.json` CAS record, requires its exact active purpose/root/child/state/version/transition, derives the child owner from the accepted profile, and rejects caller-selected owner input. The Stage verifier uses the same fixed current record with the Stage-owner purpose; issuer replacement with `INVALIDATED` or `COMPLETED` makes old cards unusable. A Stage cleanup plan requires one observation for every bound resource. Missing, dirty, unknown or SHA-mismatched resources are preserved as failures. Removal succeeds only with an explicit post-removal `exists=false` observation; verified completed keys are skipped on retry.
