...
- Phase 1 的 BusinessCard public render 使用 [query.md](./query.md) 中的 `ResolveContactActionTargets` 获取 `publicValueSummary`。
...
The exact 44-RPC matrix, `urn:oes:service:identity-service` audience, caller shapes, existing business Codes, Auth-only `identity.internal.auth_login_account.resolve` and four request tombstones are owned by [identity-service.md](../../architecture/services/identity-service.md#16-trusted-grpc-44-rpc-contractfrozen). External resolver-management methods retain their contracts; `ResolveMachinePrincipalForAuth` keeps the exact-Auth mTLS/no-Authorization pre-context policy frozen in its contract. The three Auth login account resolvers consume a normal target-audience INTERNAL ExecutionToken and never create a BUSINESS grant for the fixed Auth principal.
