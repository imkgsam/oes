...
- 作用：为已单独声明的 BusinessCard management/readiness 或 legacy BUSINESS compatibility consumer 解析 Contact Action 的 Contact Asset 引用，返回 public-safe value summary；Public Business Card 公开渲染使用 §4.3 的 dedicated resolver。
- 调用方：
  - BusinessCard 管理端 readiness diagnostics
  - 已声明的 legacy BUSINESS compatibility consumer
- 请求关键字段：
...
- `tenantId` 必须来自已声明的 management/compatibility caller 受控上下文，不来自匿名访问者任意输入。
...
- request：`tenant_id=1`, `employee_id=2`, `repeated ContactActionTargetRef target_refs=3`。Tenant/employee 来自 Public Entry service-owned BusinessCard record；refs 来自该 card 的 Contact Action config。它们是 owner lookup inputs，不建立 tenant/account authority。
...
- response：`available=1`, `tenant_id=2`, `employee_id=3`, `account_id=4`, `display_name=5`, `repeated ResolvedContactActionTarget targets=6`, safe `reason_code=7`。`ContactActionTargetRef` 与 `ResolvedContactActionTarget` 复用 §4.1-§4.2 的既有 action/ref compatibility 与 `ContactAssetPublicValueSummary` public-safe shape；response 中每个 target 必须保持对应 request action/ref，不得重排成无法归因的裸值。
...
以下接口是历史 work email / work phone 窄口径查询。Phase 1 Public Business Card 公开链路只使用 dedicated `ResolvePublicBusinessCardIdentity`；`ResolveContactActionTargets` 只服务已单独声明的 BUSINESS/management compatibility consumer。管理端或兼容页面如需列出资产，应逐步迁移到统一 Contact Asset list contract。
