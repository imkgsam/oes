...
> `hr-service` 的服务设计唯一真相源是 [hr-service.md](../../architecture/services/hr-service.md)。涉及角色、权限、grant、AccountRole 或授权判定的服务设计边界，以 [permission-service.md](../../architecture/services/permission-service.md) 为准。

## 1. 目的

本目录用于提供 `hr-service` minimum 第一阶段的黑盒接口文档，只描述调用入口、请求响应、错误语义与调用方约束。

`Employee`、`Employment`、员工生命周期、正式 `人 -> org` 归属与 onboarding owner 边界不在本目录重新定义；如需变更这些设计，必须先更新 [hr-service.md](../../architecture/services/hr-service.md)，再同步调整 contract。
...
- `Employee / Employment` owner、正式 `人 -> org` 真相与 HR onboarding owner 边界以 [hr-service.md](../../architecture/services/hr-service.md) 为准；本文只约束接口如何消费这些稳定语义。
- `Tenant / OrgUnit / org tree` 的服务边界以 [tenant-org-service.md](../../architecture/services/tenant-org-service.md) 为准；本文只描述 HR 对 `OrgUnit` 的引用方式。
