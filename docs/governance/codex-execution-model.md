...
Cleanup is disposal, not delivery or defect discovery. Its executable is isolated from routing, task creation, product writes, PR publication, merge, and CI. The authorization binds terminal state and each exact owner ref/clone/worktree/scratch/DP resource, including CO-owned aggregate resources. Each filesystem target is reopened as the same canonical physical path so a symlink or substituted identity is preserved rather than removed. It permits no cleanup branch and no repository-content diff; DP/ADP files are external stable artifacts.
