...
Phase 2 begins only after the Auth/Identity/Permission direct MACHINE foundation is available to the deployment: each fixed caller workload receives an Identity-provisioned SYSTEM Machine Principal, SPIFFE binding and non-secret exact selector, exchanges current mTLS + selector through Auth STS for `aud=urn:oes:service:party-service`, and sends the certificate-bound ET over mTLS. Final acceptance exercises provisioned selector, live Identity resolution, Permission decision, STS audience and certificate bindings end to end. This sequencing changes no Party RPC, business rule, persistence, schema, or outbound collaboration and does not migrate any other RPC in the five caller services.
