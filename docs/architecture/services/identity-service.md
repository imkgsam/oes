...
- Item Master 的上游租户证明不修改本 owner fact：MES、WMS、Procurement 与 SRM 仍解析为 tenantless `SYSTEM` Machine Principal，Identity 不把上游 HUMAN Token 的 tenant 写入 Machine Principal、`MachineWorkloadBinding` 或 resolver response。该 tenant 只由 Auth 验证后用于一次 target ExecutionToken execution context；Identity 不验证、保存或签发上游/目标 Token。
