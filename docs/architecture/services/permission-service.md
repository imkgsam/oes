...
- 用户或机器认证、API Key credential、MFA、OTP、challenge、session、refresh token、access token、STS 或 ExecutionToken 签发语义；这些认证与 credential 真相归 `auth-service`。MACHINE principal/binding owner facts 与 selector provisioning 归 `identity-service`。
...
Direct MACHINE root 进入 `ResolvePrincipalAuthorization` 前，Auth 必须已验证 current mTLS workload/leaf，并通过 Identity-owned `ResolveMachinePrincipalForAuth` live decision 得到 active principal、scope、tenant/org 与 exact `MachineWorkloadBinding` reference/version。Permission 只消费这些 owner-resolved typed facts 并独立计算当前 MACHINE BUSINESS grant；它不接收 selector、leaf certificate，不解析 SPIFFE-to-principal mapping，不读取 Identity/Auth storage，也不允许 selector 或 caller 自报的 tenant/grant 扩大授权。Identity resolution 缺失、stale、mismatch 或不可用时 Auth 不得调用本判定或签名。

两个判定都采用全量申请语义：requested Code 必须去重、规范排序且 kind 一致，只有全部获准时 `allowed=true`。未知、不可分配、部分批准、scope / tenant / audience / principal / delegation mismatch、stale decision 或依赖不可用都 fail closed；Permission 返回精确 granted / denied Code、安全 reason category、decision reference 与 `authzVersion`，Auth 不做部分签发。Permission 记录判定审计但不记录 source credential、MACHINE selector 或 Token 正文，也不签发或存储 ExecutionToken；`ResolvePrincipalAuthorization` 只按受保护 resolver 契约验证随请求提交的 ExecutionToken，不取得其签发或授权真相所有权。
...
Permission audience 固定为 `urn:oes:service:permission-service`。本组覆盖 2026-07-27 baseline 66 RPC。其后已集成的 `ResolveExternalMachineAuthorizationSnapshot`, `ResolvePrincipalAuthorization`, `ResolveWorkloadIssuance`, `ResolveDelegatedAuthorization`, `RevokePrincipalRoleBinding` 保持既有 security contract，尤其 `ResolveWorkloadIssuance` 仍是唯一 exact-Auth mTLS bootstrap，不计入本组 66。

| 类别 | RPC（数量） | execution / terminal | Code 与 caller rule |
| --- | --- | --- | --- |
| `INTERNAL` | `CheckPermission`, `BatchCheckPermission`, `ResolveAccountTerminalAccess`, `CheckResource`, `BuildQueryScope`, `GetAccountAccessSummary`, `ResolveAccountNavigation`（7） | exact `HUMAN_OBO` where an upstream user proof exists; exact Auth/Public Entry pre-auth/public `SYSTEM MACHINE` only where listed; no direct end-user gRPC | six transport Codes: `permission.internal.permission.check`, `permission.internal.account_terminal_access.resolve`, `permission.internal.resource.check`, `permission.internal.query_scope.build`, `permission.internal.account_access_summary.resolve`, `permission.internal.account_navigation.resolve`; exact Gateway/Auth/Public Entry/Collaboration actor-workload allowlists |
| `BUSINESS` | `CreatePermission`, `BatchCreatePermissions`, `UpdatePermission`, `DeletePermission`, `GetPermissionById`, `GetPermissionByCode`, `ListPermissionsPaged`, `ListPermissionRoles`, `CreateRoleTemplate`, `CreateRoleInstance`, `GetRoleTemplateById`, `UpdateRoleTemplate`, `DeleteRoleTemplate`, `SetRoleTemplateEnabled`, `ListRoleTemplatePermissions`, `AssignRoleTemplatePermission`, `RevokeRoleTemplatePermission`, `CreateRoleInstanceFromTemplate`, `UpdateRole`, `SetRoleEnabled`, `DeleteRole`, `GetRoleById`, `ListRoleInstances`, `ListRoleTemplates`, `ListRolePermissions`, `AssignRolePermission`, `RevokeRolePermission`, `AssignAccountRole`, `EnsureTenantRoleInstanceFromTemplate`, `GrantInitialAccessForEmployeeAccount`, `GrantInitialAccessForTenantAccount`, `RevokeAccountRole`, `ListAccountRoles`, `ListRoleAccounts`, `GetAccountRoleSelection`, `SetAccountRoles`, `ListAuditEvents`, `ListNavigationEntries`, `GetNavigationEntry`, `CreateNavigationEntry`, `UpdateNavigationEntry`, `GetRoleNavigation`, `SetRoleNavigationVisibility`, `SetRoleLandingPolicies`, `SyncRoleNavigationFromTemplate`, `ResolveNavigationPreview`, `GetRoleTerminalAccess`, `SetRoleTerminalAccess`, `GetAccountTerminalAccess`, `ReplaceAccountTerminalAccessOverride`, `DeleteAccountTerminalAccessOverride`（51） | direct `HUMAN` or exact HR/TenantOrg onboarding `HUMAN_OBO`, `WEB` | current management/role-template/role-instance/account/navigation/terminal-access Code selected literally per RPC |
| `BUSINESS` | `CreatePolicyInstance`, `GetPolicyInstance`, `ListPolicyInstances`, `SetPolicyInstanceEnabled`, `EvaluatePolicyInstancePreview`, `GetPolicyById`, `ListPoliciesPaged`, `ListPoliciesByPermission`（8） | `HUMAN`, `WEB` | current `permission.policy.*` and management Codes; Gateway only |

The 59 BUSINESS mappings preserve the current literal controller declarations: permission CRUD uses `permission.create/update/delete/list/get_by_id/get_by_code`; role-template methods use the exact `permission.role_template.*` action; role-instance and role navigation methods use the exact `permission.role_instance.*` action; account-role/onboarding methods use `permission.account.assign_roles`, reads use `permission.account.get_roles`; navigation methods use the exact `permission.navigation.*` action; terminal methods use `permission.terminal_access.view/role.manage/account.manage`; policy methods use `permission.policy.create/update/list`. `RevokePrincipalRoleBinding` and the four later decision RPCs remain outside the 66 and retain their integrated declarations. No request selects its admission Code.

Permission does not obtain tenant authority from a caller's body merely because it evaluates a decision. The authorization-decision requests intentionally retain subject/target tenant, org, Code, resource and workload inputs that are part of the Permission-owned decision contract; they must be bound to the already verified Auth/workload caller and are not gRPC admission identity. Only `ListAuditEvents.operator_id=5/tenant_id=6/org_id=7` are removed and reserved as three legacy authority fields. No role, policy, scope, grant, schema or decision algorithm changes in this migration.

Auth login/session owner-fact resolution uses three target-owned INTERNAL Codes: `identity.internal.auth_login_account.resolve`, `hr.internal.auth_login_employee.resolve`, and `tenant_org.internal.auth_session_tenant_lifecycle.resolve`. Permission owns their catalog metadata and exact workload -> audience -> Code issuance policy, evaluates them through existing `ResolveWorkloadIssuance`, and records the workload decision reference/version. These Codes are `WORKLOAD_POLICY`-only SYSTEM capabilities and never require or create a `PrincipalRoleBinding` for the fixed Auth Machine Principal. `ResolveWorkloadIssuance` remains the sole pre-Token bootstrap method; target resolvers still require their normal certificate-bound target-audience ExecutionToken and exact local method declaration.

Public Business Card owner-fact resolution uses the same issuance primitive with a separate exact caller and three separate target Codes: `hr.internal.public_business_card_employee.resolve`, `identity.internal.public_business_card_identity.resolve`, and `tenant_org.internal.public_business_card_organization.resolve`. Only the registered Public Entry workload may request the corresponding Code for its literal target audience. These are likewise `WORKLOAD_POLICY`-only SYSTEM capabilities and never read or create a Public Entry `PrincipalRoleBinding`, tenant role or BUSINESS grant. In particular Permission must not make `hr.employee.get_by_id`, `identity.account.list`, `identity.account.self.read`, `tenant_org.tenant.get_by_id` or `tenant_org.org_unit.list_tree` available to that SYSTEM principal. Selector/owner admission remains target-owned; the exact flow is [Public Business Card owner-fact resolution](../collaborations/public-business-card-owner-facts.md).
