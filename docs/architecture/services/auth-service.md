...
- Identity decision 必须确认 Machine Principal active、type/scope/tenant 与适用 org reference 有效，credential reference 唯一对应同一 active binding，且 binding 的 SPIFFE ID 与版本匹配。Auth 只从该 owner decision 派生 ExecutionToken 的 `sub`、`principal_type=MACHINE` 与 scope；普通 MACHINE exchange 的 tenant/org 也只来自该 decision。唯一窄化例外是下述 Item Master upstream-proof profile：SYSTEM principal 仍 tenantless，但本次 execution tenant 来自另行验证的上游 HUMAN ET。caller 或 credential 中重复的主体字段不能覆盖 owner facts。
...
Item Master 的三个 tenant-scoped INTERNAL 资格查询使用一个窄化的上游租户证明组合，不改变 Machine Principal owner 语义：MES、WMS、Procurement 与 SRM 的 source credential 仍只解析为 tenantless `SYSTEM MACHINE`。当这些 workload 为 `aud=urn:oes:service:item-master-service` 的冻结 INTERNAL Code 换票时，Auth 还必须通过独立 transport-private carrier 验证当前 Auth 签发的上游 HUMAN ExecutionToken；该 Token 的 signature、issuer、time、`principal_type=HUMAN`、非空 tenant、`aud`（精确等于当前调用服务自身 audience）以及当前调用服务 workload 边界必须全部成立。request/body、ordinary metadata、caller-local `tenantId` 与 Machine credential 都不能提供或覆盖这个 tenant authority。

Auth 组合后的目标 Token 仍使用 Identity owner decision 中的 Machine `sub`、`principal_type=MACHINE` 与 `scope=SYSTEM`，只把已验证上游 Token 的 `tenant_id` 作为本次 execution context 写入；`client_id`、target `aud` 与 `cnf` 绑定当前直接调用 workload 和 leaf certificate。目标 Token `exp` 不得晚于上游 Token `exp`，Auth 的 deployment-owned workload registry 必须同时固定当前 workload 的自身 audience 与哪些 target audience 要求该 proof。Auth 在返回 Token 前持久化 upstream `jti` -> target `jti`、tenant、workload、audience、Permission decision reference 与 trace correlation；审计或 proof dependency 失败时不返回 Token，且不记录 bearer。无可信上游 HUMAN Token 的后台任务不获得 body/local fallback，留待独立设计。
