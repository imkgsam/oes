...
- After Auth has verified the credential, Integration Machine and tenant, it obtains the machine's current external authorization snapshot only through Permission Service's trusted machine-authorization contract. The snapshot contains the effective, externally eligible `BUSINESS` Permission Code subset and an Auth-auditable authorization version; Auth never reads Permission storage or accepts a caller-supplied capability, role, Permission Code or tenant to construct it.
