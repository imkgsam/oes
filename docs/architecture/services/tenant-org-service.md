...
- 组织节点与当前租户内 `ORGANIZATION` TenantParty 的可选受控关联
...
- `Employee / Employment` 与员工任职真相；这些以 [hr-service.md](./hr-service.md) 为准
...
- 为 `hr-service` 提供可被正式任职关系引用的 `OrgUnit` 真相；HR 任职设计以 [hr-service.md](./hr-service.md) 为准
- 在必要时允许组织节点受控关联到 `party-service` 的当前租户 `ORGANIZATION` TenantParty，但不替代主体主数据
- `organizationTenantPartyId` 的基础语义是“组织节点对当前租户内组织主体的可选正式引用”，不是所有 `OrgUnit` 都默认拥有的字段语义
- 当前第一阶段只允许 `ROOT` 与 `BRANCH` 节点持有 `organizationTenantPartyId`；`DEPARTMENT`、`TEAM`、`OTHER` 不得绑定组织主体
- `tenant-org-service` 负责在写入口校验 `organizationTenantPartyId` 是否可被当前节点类型持有，并通过 `party-service` 只读 query 校验目标主体是否存在、是否为可绑定的组织主体、以及是否处于可引用状态；组织主体边界以 [party-service.md](./party-service.md) 为准
- 拥有 tenant onboarding process manager 状态，负责编排创建 tenant、root org、当前租户 `ORGANIZATION` TenantParty、first admin account、first admin employee 与初始访问权；下游事实仍由各 owner service 拥有
...
- `depth`
- `sortOrder`
- optional `organizationTenantPartyId`
...
接口字段、错误语义与调用上下文以 [tenant-org-service contracts](../../contracts/tenant-org-service/README.md) 为准；contracts 只能描述黑盒接口语义，不重新定义本服务核心边界。
...
- `hr-service`
  - 在需要基于正式员工任职形成 org-based scope 时提供人员任职事实；服务边界以 [hr-service.md](./hr-service.md) 为准
- `permission-service`
...
- `tenant-org-service` 不拥有角色、权限、policy 与授权判定真相；这些以 [permission-service.md](./permission-service.md) 为准
...
- 不拥有 `Employee / Employment`、岗位、汇报关系或薪酬考勤等 HR 语义；这些以 [hr-service.md](./hr-service.md) 为准
