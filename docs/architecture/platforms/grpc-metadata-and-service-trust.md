...
- 高危操作使用 [DG-4 冻结的 ActionGrant](../collaborations/delegated-execution-and-action-grant.md)：它绑定 operation、target、canonical input digest 与 idempotency reference，并由目标服务随业务写入一次性消费。
...
3. External API Key security contract 已冻结：外部 Integration credential 的创建、交换、轮换与开放以 [External API Key Security Collaboration](../collaborations/external-api-key-security.md) 为准。Gateway locally verifies five-minute Auth-signed external access tokens; credential revocation stops new exchange and does not add a DG-2 external-token deny-cache dependency.
4. DELEGATED execution 与 ActionGrant contract：已由 [ADR 0016](../../adr/0016-delegated-execution-and-action-grant.md) 冻结；它解除 AI delegation 与一次性高危授权的设计阻塞，但不替代 DG-1 的签名 / workload binding 或 DG-2 的紧急撤销设计。
...
- [Delegated Execution And ActionGrant Collaboration](../collaborations/delegated-execution-and-action-grant.md)
