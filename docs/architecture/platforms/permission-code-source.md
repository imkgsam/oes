...
> `permission-service` 的服务职责、Role、Grant 与 Policy 以 [permission-service.md](../services/permission-service.md) 为准。本文只冻结 Permission Code 的代码语义源、metadata 与运行时同步方向。
...
- `ResolveMachinePrincipalForAuth` 改为 Identity exact-Auth mTLS/no-Authorization pre-context owner resolver，因此不再消费 `identity.internal.machine_principal.resolve`；该 Code 在 additive migration drain 完成后从 catalog、policy 与 runtime declaration 删除，不转授给任何 role 或 workload。
- Auth 登录/会话安全事实固定使用三个 target-owned INTERNAL Code：`identity.internal.auth_login_account.resolve` 精确映射 `ListAuthLoginAccountCandidates` / `ResolveAuthLoginAccount` / `ResolveAuthEmployeeLoginAccount`，`hr.internal.auth_login_employee.resolve` 精确映射 `ResolveAuthLoginEmployee`，`tenant_org.internal.auth_session_tenant_lifecycle.resolve` 精确映射 `ResolveAuthSessionTenantLifecycle`。三者均为 `kind=INTERNAL`、`assignableTo=[WORKLOAD_POLICY]`、`allowedScopeLevels=[SYSTEM]`、`externalApiEligible=false`，仅允许环境注册的 exact `auth-service` workload 为 exact owner audience 申请；它们不进入 HUMAN/MACHINE role、external JWT、wildcard policy 或 `system.admin`，也不为固定 Auth Machine Principal 建立登录用 BUSINESS grant。
- MACHINE binding management 只保留 Identity-owned BUSINESS Code `identity.machine.workload_binding.manage`，保护 `EnrollMachineWorkloadBinding` / `DisableMachineWorkloadBinding`。Auth-owned `auth.machine_workload_source_credential.revoke` 随旧 Revoke RPC 在 migration drain 后删除。deployment provisioner 使用独立 deployment admission 与 Identity-local audit，不把 provisioner 变成 BUSINESS RPC 或 Permission Code。
- 该 Code 只在 Permission canonical catalog source 中维护，并通过现有 generator 与 catalog/codegen tests 回写 Common；generated Common 文件不得手工维护，也不得借此复制 Auth credential profile 或 Identity binding 语义。
