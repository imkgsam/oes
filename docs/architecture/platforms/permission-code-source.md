...
- 内部 MACHINE root resolution 已实现：独立技术调用 Code `identity.internal.machine_principal.resolve` 精确映射 `IdentityQueryService.ResolveMachinePrincipalForAuth`。它只允许准确 `auth-service` workload 通过 `ResolveWorkloadIssuance` 为 `aud=identity-service` 申请并绑定当前 Auth leaf certificate；不进入 HUMAN/MACHINE role、external JWT、API-key exchange 或 wildcard workload policy，也不把 Identity resolver 变成第二个 mTLS-only bootstrap primitive。
