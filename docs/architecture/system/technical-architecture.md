...
> 涉及 permission-service 的服务职责、核心对象或 owner 边界时，以 [permission-service.md](../services/permission-service.md) 为准；本文只描述项目级技术架构。
...
Event Bus 的 CloudEvents envelope、provider、按 service 归属的 common code contract、transactional outbox / consumer inbox、DLQ、replay、tenant 与运行边界，以 [event-bus-and-outbox.md](../platforms/event-bus-and-outbox.md) 为准；业务 event type、payload 与版本继续以 `docs/contracts/events/` 为准。
...
负责统一表示交易与法律主体；具体职责、核心对象、`TenantParty` 引用规则与 non-goals 以 [party-service.md](../services/party-service.md) 为准。
