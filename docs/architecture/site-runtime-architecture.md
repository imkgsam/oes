...
本文用于冻结 OES 与外部网站之间的顶层协作方式。`site-service` 的服务职责与 P1 对象边界以 [site-service.md](/Users/acehood/Documents/GitHub/oes/docs/architecture/services/site-service.md) 为准。
...
- `site-service` 负责站点注册、语言、域名、凭证、站点产品发布配置、站点私有 Blog / News、public view、同步、webhook 与审计。
...
```text
OES Admin 保存产品站点配置 / Blog / News 草稿
  ↓
...
发现 latest publishVersion = 103
  ↓
拉取 101 到 103 的 changed resource list
  ↓
批量拉取 changed resources 的最新 publish view，或在必要时拉取 snapshot
```
...
`@oes/site-runtime-kit` Phase 1 模块结构与公共边界已回写到 [site-runtime-kit.md](/Users/acehood/Documents/GitHub/oes/docs/architecture/site-runtime-kit.md)。
