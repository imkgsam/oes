...
- 内部 MACHINE root resolution 使用独立技术调用 Code `identity.internal.machine_principal.resolve`，精确映射既有 `IdentityQueryService.ResolveMachinePrincipalForAuth`。它只允许准确 `auth-service` workload 通过 `ResolveWorkloadIssuance` 为 `aud=identity-service` 申请并绑定当前 Auth leaf certificate；不进入 HUMAN/MACHINE role、external JWT、API-key exchange 或 wildcard workload policy，也不把 Identity resolver 变成第二个 mTLS-only bootstrap primitive。
- 该 Code 的未来最小 lease 是 Permission canonical catalog source、由 generator 产生的 `src/common/src/authorization/permission-codes/identity/internal.permission-codes.ts` 与 `identity/index.ts` export，以及对应 catalog/codegen tests。generated Common 文件不得手工维护；除公共 definition 类型确实不足外不涉及类型文件，也不得借此复制 Auth credential profile 或 Identity binding 语义。
