...
> 涉及 permission-service 的审计样板、权限审计或授权判定审计边界，以 [permission-service.md](../architecture/services/permission-service.md) 为准。本文只记录可观测性与审计基础实施路径。

## 1. 目标

将 [observability-and-audit.md](../architecture/platforms/observability-and-audit.md) 中确认的方案拆成可执行实施路径。
