...
| `main` | `65e49258a0dc57b7daf3d40d5e8a63ea94dfc116` | 当前 `a82e5ea69a7773d4e0e8f5a91dcdf7a599897c1d`；AI、Principal Authorization、ActionGrant design、Permission、GRPC carrier、EXEC-CRYPTO HUMAN、完整MACHINE source-verifier及GRPC Asset token-only cutover经唯一I&V lane ff-only推进 |
| `origin/main` | `65e49258a0dc57b7daf3d40d5e8a63ea94dfc116` | 当前本地 remote-tracking ref 与 I&V 记录的 remote `refs/heads/main` 均为 `a82e5ea69a7773d4e0e8f5a91dcdf7a599897c1d` |
...
| newly integrated clean worktrees | 4 | GRPC carrier `3e263e50…`、EXEC-CRYPTO HUMAN foundation `1ca24f41…`、MACHINE source-verifier `02457959…`与GRPC Asset `a82e5ea6…`；已独立验收并进入main，等待final cleanup manifest |
...
| active current-main implementation worktree | 0 | GRPC Asset source已clean at integrated `a82e5ea6…`并标记`MERGED_WAITING_FOR_USER_CLEANUP`；当前无active implementation writer |
| **worktree total** | **38** | MACHINE与Asset source worktrees均clean并等待final cleanup；root与Program Control clean。删除/clean/reset数为0 |

branch refs 共 32：25 个 branch HEAD 已是 current main ancestor（Asset rebuild branch当前等于main）；7 个非 ancestor refs 已全部分类为 deferred ActionGrant runtime、AI legacy accepted evidence、API-KEY rejected prototype evidence、EXEC-CRYPTO legacy checkpoint、GRPC Asset legacy candidate、原 GRPC carrier candidate 与 Program Control migration ledger。MACHINE branch 已是 current main ancestor。不存在未分类 branch ref。

任务处置快照：handoff registry 仍登记 101 个 legacy formal A/* tasks，最终均须在直接证据消费后归档；最近 50 项应用快照可见其中 24 个 legacy A/* tasks，另可见 legacy Global Command 1 个。MIG-D04 导致部分 capability 缺少逐 task IDs，因此当前 thread archive manifest 尚未完整，不能把 101 项声明为已具备逐项归档条件。本轮未唤醒或删除旧 capability 资源；checker为0。AI、Permission、GRPC rebuild、EXEC-CRYPTO remediation、MACHINE replacement owner及四个Asset执行owners均已完成/退役并归档。持久I&V保留为idle后续复用任务，Unified Design idle，Program Control继续保留。
...
GRPC Asset implementation `a82e5ea69a7773d4e0e8f5a91dcdf7a599897c1d` 已 `ACCEPTED_AND_INTEGRATED`；root/local origin/remote main同SHA且clean，MIG-D21/D22/D23关闭。source branch/worktree clean并标记`MERGED_WAITING_FOR_USER_CLEANUP`；所有Asset implementation owners已归档，persistent I&V保留idle复用。本迁移排序的下一非AI runtime目标是`site-service` trusted-gRPC/current-main caller inventory与token-only cutover准备；在派发实现前须复核Site 59 Admin + 7 Runtime RPC、Gateway/Site->Asset callers，以及Site Inspiration的Event/outbox与CDN/purge前置。若CDN/purge ownership或wire仍未冻结，先回同一Unified Design；不得以普通URL、直接对象存储或no-op purge绕过。AI/ActionGrant runtime保持deferred，API-KEY `755d857a…`不进入main；无checker，当前不执行cleanup。Program Control ledger仍只在`codex/oes-program-control-migration`本地提交，待final ledger integration gate后才进入main并清理worktree。
