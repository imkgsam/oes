...
- A follow-up PDA auth login live smoke was run after adding deterministic local seed data for the Meilong PDA smoke account and PDA Terminal Access Policy. The smoke created an enrollment over `TerminalDeviceEnrollmentService.CreateEnrollment`, activated the PDA through `POST /api/v1/pda/device/enroll`, logged in through `POST /api/v1/pda/auth/login`, decoded the returned access token, and bootstrapped the authenticated session through `GET /api/v1/pda/session/bootstrap`.
- Observed auth results: enrollment `decisionCode=ALLOW`; login returned `SUCCESS` and `nextStep=NONE`; the HTTP session carried `terminal=PDA`, `terminalDeviceId`, and `deviceBoundTenantId`; the decoded access token carried the same `terminalDeviceId` and `deviceBoundTenantId`; bootstrap returned the same account, tenant, session device id, device id, and `decisionCode=ALLOW`.
