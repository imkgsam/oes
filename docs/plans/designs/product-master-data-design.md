...
> 2026-05-10 状态：本 workspace 已被 [item-master-service.md](/Users/acehood/Documents/GitHub/oes/docs/architecture/services/item-master-service.md) superseded。本文只保留历史讨论脉络，不再作为 item-master-service、ProductModel、ItemModel、Item、BOM、Packaging 或 capability 的稳定真相源。
>
> 2026-05-04 收口：当前阶段不建立独立 `product-service`。`item-master-service` 承担 Item 基础主数据与类似 product master 的职责；MES 生产规格真相以 [mes-service.md](/Users/acehood/Documents/GitHub/oes/docs/architecture/services/mes-service.md) 中的 `ProductionSpec` 为准，并通过 active + manufacturable `Item` 引用 Item Master 准入边界。
...
| 2026-05-04 | 当前阶段不建立独立 `product-service`；`item-master-service` 继续作为 Item / product-master-like 基础主数据服务，MES `ProductionSpec` 归 `mes-service`。 | Item Master、MES、产品主数据服务化边界 | `docs/architecture/services/item-master-service.md`; `docs/architecture/services/mes-service.md`; `docs/contracts/mes-service/**` |

## 5. 开放问题

| 日期 | 问题 | 为什么未冻结 | 下一步 |
| --- | --- | --- | --- |
| 2026-04-22 | 哪些对象属于共享主数据，哪些对象属于 `sales / mes / wms` 的运行事实 | `Item / ProductionSpec` 已收口到各自服务真相源，但包装定义、营销展示对象、客户专属组合仍未冻结 | 后续按具体主线分别回写到 Item Master、MES、Sales 或 WMS |
| 2026-04-22 | 包装定义、组合规则、销售对象之间的最小映射模型是什么 | 历史讨论曾使用 `ItemComposition`；新 truth 已改为通过 BOM 与 sales transaction snapshot 分别表达 | 基于报价、履约与包装作业场景继续收敛 |
...
- `item-master-service` 承担 `ItemModel`、`Item`、attribute、BOM、Packaging、capability、`ItemCategory` 与供应商型号映射。
- `mes-service` 承担 `ProductionSpec` 与制造执行相关设计；本文只引用 MES 真相源，不重新定义其对象结构。
