...
- `party-service` 第一阶段 registration、query、merge runtime / proto / Prisma schema 与测试样板已经落地，当前阶段不再是纯设计态。
...
- 当前 feature packet 明确引用 `party-service` 职责卡与 ADR 0003，不重复定义服务边界。
