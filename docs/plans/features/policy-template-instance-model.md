...
- 采购 category、销售国内/国外范围、客户/供应商可见范围、多仓库/多工厂范围，优先通过 `ACCOUNT` 级 policy instance 或后续 access profile 表达。
...
params:
  field: marketType
  allowedValues: [domestic]
enabled: true
```

使用方式：

- 销售订单列表 / search：`buildQueryScope` 生成 `marketType IN [domestic]`。
- 创建 / 修改订单：`checkResource` 校验订单 market facts。
