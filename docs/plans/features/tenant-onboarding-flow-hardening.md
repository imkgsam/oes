...
- 不强行创建 employee / employment / account-org membership。
- 不复用 employee onboarding grant 语义给首租户管理员授权。
...
- `permission-service` 继续拥有 tenant.admin role instance 与 account role grant。
...
当前文档冻结的是目标 contract 语义，尚未代表 proto / runtime 已实现。
...
| tenant-org implementation owner | 实现 onboarding run、step 状态与 Saga 编排 | `src/services/system/tenant-org-service/**`, `src/common/src/contracts/tenant_org_service/**` | feature packet + tenant-org contract | 可重试 onboarding API 与测试 | pending |
| permission implementation owner | 增加非 employee 语义的 tenant account initial grant 能力 | `src/services/system/permission-service/**`, `src/common/src/contracts/permission_service/**` | permission contract | role ensure + grant 能力与测试 | pending |
| downstream integration owner | 补 party / identity / auth 必要幂等或响应字段 | 对应 system service contract / implementation | feature packet | 下游稳定协作能力 | pending |
| frontend owner | 将 tenant create modal 升级为 onboarding wizard/result | `app/web/apps/tenant-web/**` | api-gateway contract | 可用 wizard、失败展示、retry 入口 | pending |
| review owner | 检查是否越界、是否可恢复、是否复用 employee 语义 | 只读全局，必要时最小文档收口 | 实现结果与验证 | review 结论 | pending |

## 7. 当前 slice

- slice:
  - contract / feature packet freeze
- status:
  - design confirmed
...
9. `tenant-org-service` 调 `auth-service.BootstrapUserLoginMethods`。
10. 若请求要求，`tenant-org-service` 调 `auth-service.RequirePasswordSetup`。
11. `tenant-org-service` 调 `permission-service.EnsureTenantRoleInstanceFromTemplate`，确保 `tenant.admin` role instance。
12. `tenant-org-service` 调 `permission-service.GrantInitialAccessForTenantAccount`，授予首管理员。
13. `tenant-org-service` 标记 onboarding 成功，并返回聚合结果。
...
| 2026-05-02 | party-service 写接口是否首版补 `idempotency_key` | Blocker-Now | 影响重复提交和响应丢失后的恢复质量 | contract 阶段列为推荐正式方案；实现前确认是否同批落地 | party-service contract | open |
| 2026-05-02 | identity 创建账号是否返回 person tenant-party | Blocker-Now | 影响 onboarding result 完整展示和恢复记录 | 优先扩展 response 或增加 hydration query | identity-service contract | open |
| 2026-05-02 | tenant.admin role instance ensure 接口命名 | Blocker-Now | 影响 permission proto 与长期 grant 语义 | 暂定 `EnsureTenantRoleInstanceFromTemplate` | permission contract | open |
| 2026-05-02 | 首管理员是否必须 password setup | Product Decision | 影响 auth 调用链和前端提示 | 默认建议 require password setup；实现前由产品口径确认 | auth contract / gateway contract | open |
...
- 首管理员拥有 tenant scoped `tenant.admin` role instance grant。
