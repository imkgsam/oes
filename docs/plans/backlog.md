...
| 2026-06-24 | CRM 来源记录手动管理闭环：手动新增来源、设置主来源、来源证据校验与审计 | Product Deferred | 当前 `crm-source-record-closure` 只推进 Phase 1 只读闭环。手动创建来源与设置主来源涉及写契约、来源证据最低要求、`crm.source.manage` 权限、审计明细、AI / 自动化来源写入是否复用同一命令等语义，尚未讨论和冻结，不应混入 P1 实现。 | 到达触发条件后拆成独立 CRM feature packet：1. 冻结 `AddSourceRecord` 与 `SetPrimarySourceRecord` 命令语义；2. 冻结各 sourceType 的服务端证据校验规则；3. 明确权限码、审计事件字段与 operator / trace 传播；4. 冻结 Gateway mutation endpoint 与 tenant-web 手动管理 UI。 |
