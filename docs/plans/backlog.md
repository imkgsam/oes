...
| 2026-04-22 | `entity` 历史遗留清理：代码、契约、文档、命名与运行配置统一收口到 `party-service` | Platform Deferred | 当前目标架构已冻结为 `party-service`，并且 `identity.User -> partyId` 主链已经落地；但仓库中仍残留 `entity-service` / `entityId` / `entityType` 等历史命名、旧契约、旧文档引用与兼容实现。该事项属于跨服务治理清理，不宜在当前活跃 feature 中穿插推进，否则容易把正常业务主线和迁移治理混在一起。 | 到达触发条件后升级为独立 cleanup / migration plan：1. 盘点 `src/**`、`docs/**`、proto、generated、service discovery、环境配置与测试夹具中的 `entity` 残留；2. 区分“必须保留的兼容层”和“可直接迁移的正式路径”；3. 先冻结 `party-service` 对应替代边界与命名表；4. 分批迁移代码、契约、文档与运行配置，并为每批明确回归验证与回滚策略。 |
