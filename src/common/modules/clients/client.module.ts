...
export class ClientModule {
  static register(): DynamicModule {
    const providers: Provider[] = Object.entries(SERVICE_ENDPOINTS_CONFIG).map(([key, endpoint]) => ({
      provide: SERVICE_CLIENT_TOKENS[key],
      useValue: getOrCreateClient(key, endpoint),
    }));

    const logger = new Logger(ClientModule.name)
    logger.log('ClientModule initialized with endpoints: ' + Object.keys(SERVICE_ENDPOINTS_CONFIG).length);
