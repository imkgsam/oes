...
describe('Public Business Card owner-fact permission catalog and workload policy', () => {
  ...
  it('binds only Public Entry to the three exact audience/code tuples', () => {
    ...
    const entries = policies.filter((entry: any) =>
      entry.originalWorkloadSpiffeId.endsWith('/public-entry-service')
    )
    expect(entries).toHaveLength(3)
    expect(
      entries.map((entry: any) => [entry.targetAudience, entry.permissionCodes[0]]).sort()
    ).toEqual([...exactTuples].sort())
