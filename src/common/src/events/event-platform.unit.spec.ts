...
describe('event platform CloudEvents codec', () => {
  ...
  it('propagates a valid W3C trace context only as transport headers without duplicating it into the body', () => {
    ...
    ).toContainEqual(['traceparent', '00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01'])
  })
  ...
  it('rejects invalid supplied optional attribution attributes', () => {
    expect(() => createOesCloudEvent({ ...input, actorAccountId: '' })).toThrow(
      'EVENT_ACTOR_INVALID'
    )
    const event = createOesCloudEvent(input)
    expect(() =>
      decodeCloudEvent(Buffer.from(JSON.stringify({ ...event, oesauditref: '' })), input.contract)
    ).toThrow('EVENT_AUDIT_REF_INVALID')
  })

  it('preserves the business profile by rejecting a security scope or missing aggregate attributes', () => {
    expect(() => createOesCloudEvent({ ...input, executionScope: 'TENANT' })).toThrow(
      'EVENT_BUSINESS_SCOPE_FORBIDDEN'
    )
  ...
  it.each([
    ...
    'round-trips a %s security event without fabricating a business aggregate',
    (executionScope, tenantId) => {
      ...
      expect(toNatsPublishRequest(event, securityContract).subject).toBe(
        'oes.security.events.auth.execution-token.revoked'
      )
  ...
  it.each([
    ...
    'fails closed for an illegal %s security scope and tenant combination',
    (executionScope, tenantId, code) => {
      expect(() =>
        createOesCloudEvent({
          contract: securityContract,
...
describe('event platform operations', () => {
  ...
  it('publishes validated outbox bytes without changing whitespace or field order', async () => {
    const original = Buffer.from(JSON.stringify(event, null, 2), 'utf8')
    let published: Uint8Array | undefined
    const adapter = new NatsJetStreamAdapter({
      publish: async (request) => {
        published = request.body
        return { stream: 'OES_BUSINESS_EVENTS', sequence: 5 }
      }
    })

    await expect(
      adapter.publishStored(original, {
        eventType: 'example.fact.happened',
        eventVersion: 1,
        ownerService: 'example-service',
        validateData: (data: unknown): data is { value: string } =>
    ...
    ).resolves.toMatchObject({ kind: 'ACKNOWLEDGED', sequence: 5 })
    expect(Buffer.from(published ?? []).equals(original)).toBe(true)
  })
  ...
  it.each([
    ...
    'quarantines an untrusted %s envelope before provider publish',
    async (_caseName, mutation) => {
      const calls: string[] = []
      const adapter = new NatsJetStreamAdapter({
        publish: async () => {
          calls.push('publish')
          return { stream: 'OES_BUSINESS_EVENTS', sequence: 4 }
        }
      })
      const contract = {
        eventType: 'example.fact.happened',
      ...
      await expect(adapter.publish({ ...event, ...mutation }, contract)).resolves.toMatchObject({
  ...
  it('rejects inbound tracestate without a valid traceparent before typed delivery decoding', () => {
    const encoded = encodeCloudEvent(event)
    const adapter = new NatsJetStreamAdapter({
      publish: async () => ({ stream: 'OES_BUSINESS_EVENTS', sequence: 4 })
    })
    const contract = {
      eventType: 'example.fact.happened',
      eventVersion: 1,
    ...
    expect(() =>
      adapter.decodeDelivery(
        {
          subject: 'oes.events.example.fact.happened',
          headers: [...encoded.headers, ['tracestate', 'vendor=value']],
          body: encoded.body,
          deliveryAttempt: 1,
          ack: async () => undefined,
          nak: async () => undefined,
    ...
    ).toThrow('EVENT_W3C_TRACEPARENT_REQUIRED')
  })
  ...
  it('builds the frozen Inbox identity tuple from the immutable decoded event and body digest', () => {
    ...
    expect(
      ...
      identityTuple: [
        'evt-operations',
        'urn:oes:service:example-service',
        'example.fact.happened',
  ...
  it('publishes the immutable DLQ record before terminating the original delivery', async () => {
    ...
    await expect(
      transferToDlqThenTerm({
        ...
        publishDlq: async () => {
          calls.push('publish')
          return { kind: 'ACKNOWLEDGED', stream: 'OES_EVENT_DLQ', sequence: 3, duplicate: false }
        },
        term: async () => {
          calls.push('term')
        }
    ...
    await expect(
      runSafeRedelivery({
        request: {
          ...
          eventFilter: { eventTypes: ['example.fact.happened'] },
