...
export class NatsJetStreamAdapter {
  ...
  constructor(
    private readonly client: JetStreamPublishClient,
    private readonly traceHook?: EventTraceHook
  ) {}
  ...
  async publish<TData>(
    ...
    try {
      ...
      if (outcome.kind !== 'ACKNOWLEDGED')
        this.traceHook?.onFailure?.(traceContext(event, headers), outcome.code)
      return outcome
    }
  }

  /** Publishes exact service-owned outbox bytes after validating their envelope and transport mapping. */
  async publishStored<TData>(
    body: Uint8Array,
    contract: OesEventContract<TData>,
    traceHeaders?: W3cTraceHeaders
  ): Promise<EventPublishOutcome> {
    let event: OesCloudEvent<TData> | undefined
    let headers: EventHeaders = []
    try {
      const request = toNatsPublishRequestFromBody(body, contract, traceHeaders)
      event = request.event
      headers = request.headers
      const ack = await this.client.publish(request)
      this.traceHook?.onPublish(traceContext(event, headers))
      return {
        kind: 'ACKNOWLEDGED',
        stream: ack.stream,
        sequence: ack.sequence,
      ...
      const outcome = normalizePublishFailure(error)
      if (event && outcome.kind !== 'ACKNOWLEDGED') {
        this.traceHook?.onFailure?.(traceContext(event, headers), outcome.code)
      }
      return outcome
    }
  ...
  decodeDelivery<TData>(
    ...
    return {
      event,
      bodyDigest: digestCanonicalBody(delivery.body),
  ...
  ): Promise<'ACKED' | 'RETRY_SCHEDULED' | 'REQUIRES_DLQ'> {
    if (
      outcome.kind === 'APPLIED' ||
      outcome.kind === 'DUPLICATE' ||
      outcome.kind === 'STALE_IGNORED'
    ) {
      await delivery.ack()
      return 'ACKED'
    }
...
export function normalizePublishFailure(error: unknown): EventPublishOutcome {
  if (error instanceof EventContractError)
    return { kind: 'QUARANTINED_FAILURE', code: error.code, message: error.message }
  const code = errorCode(error)
  if (
    ['MAX_PAYLOAD_EXCEEDED', 'PERMISSIONS_VIOLATION', 'BAD_REQUEST', 'INVALID_SUBJECT'].includes(
      code
    )
  ) {
