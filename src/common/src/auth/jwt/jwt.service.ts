...
export class CommonJwtService {
  ...
  // 异步验证token
  async verifyAsync<T extends object = any>(token: string, options?: JwtVerifyOptions): Promise<T> {
    return this.jwtService.verifyAsync<T>(token, options)
  }

  // 解码 token
  decode(token: string, options?: { json?: boolean }): null | { [key: string]: any } | string {
