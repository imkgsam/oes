...
export function resolvePoolConfig(
  servicePool?: GrpcPoolConfig,
  defaultPool?: GrpcPoolConfig
): ResolvedPoolConfig {
  return {
    minSize: servicePool?.minSize ?? defaultPool?.minSize ?? DEFAULT_POOL_CONFIG.minSize,
    maxSize: servicePool?.maxSize ?? defaultPool?.maxSize ?? DEFAULT_POOL_CONFIG.maxSize,
    idleTimeoutMs:
      servicePool?.idleTimeoutMs ?? defaultPool?.idleTimeoutMs ?? DEFAULT_POOL_CONFIG.idleTimeoutMs,
    acquireTimeoutMs:
      servicePool?.acquireTimeoutMs ??
      defaultPool?.acquireTimeoutMs ??
      DEFAULT_POOL_CONFIG.acquireTimeoutMs,
    healthCheckIntervalMs:
      servicePool?.healthCheckIntervalMs ??
