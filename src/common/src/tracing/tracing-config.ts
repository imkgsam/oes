...
export function resolveTraceSampleRatio(env: NodeJS.ProcessEnv = process.env): number {
...
export function resolveIgnoreIncomingPaths(env: NodeJS.ProcessEnv = process.env): string[] {
...
function clampSampleRatio(input: number): number {
  if (input <= 0) return 0
  if (input >= 1) return 1
  return input
}
