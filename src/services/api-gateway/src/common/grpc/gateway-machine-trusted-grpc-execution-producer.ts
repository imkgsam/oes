...
export type VerifiedMachineTrace = Readonly<{
  requestId: string
  traceparent: string
  tracestate?: string
}>
...
export class GatewayMachineTrustedGrpcExecutionProducer {
  ...
  async forInternalCall<T>(
    ...
    return this.context.run(trusted, () =>
      this.source.run(trace, async () =>
        callback(await this.metadata.forInternalCall(targetAudience, [code]))
      )
    )
  }
  ...
  async forBusinessCall<T>(
    ...
    return this.context.run(trusted, () =>
      this.source.run(trace, async () =>
        callback(await this.metadata.forBusinessCall(targetAudience, [code]))
      )
    )
  }
}
