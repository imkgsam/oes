...
describe('GatewayMachineTrustedGrpcExecutionProducer', () => {
  ...
  beforeEach(() => {
    process.env.GATEWAY_MACHINE_PRINCIPAL_ID = 'spiffe://oes/gateway/runtime'
  })
  afterAll(() => {
    if (original === undefined) delete process.env.GATEWAY_MACHINE_PRINCIPAL_ID
    else process.env.GATEWAY_MACHINE_PRINCIPAL_ID = original
  })
  it('accepts unsampled flags and passes tracestate into the trusted context', async () => {
    ...
    const source = { run: jest.fn((_correlation, callback) => callback()) }
    ...
    await producer.forInternalCall(
  ...
  it('rejects malformed and all-zero W3C identifiers', async () => {
    ...
    await expect(
      producer.forInternalCall(
        ...
        {
          requestId: 'request-1',
          traceparent: '00-00000000000000000000000000000000-0000000000000001-00'
        },
        async () => undefined
      )
