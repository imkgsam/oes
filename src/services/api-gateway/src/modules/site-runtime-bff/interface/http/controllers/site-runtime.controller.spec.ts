...
import { SiteRuntimeController } from './site-runtime.controller'
...
describe('SiteRuntimeController', () => {
  ...
  it('marks Site-facing endpoints public for JWT while keeping signed material delegation explicit', () => {
    ...
    expect(reflector.get(IS_PUBLIC_KEY, SiteRuntimeController.prototype.getLatestPublishState)).toBe(true)
    expect(reflector.get(IS_PUBLIC_KEY, SiteRuntimeController.prototype.listChangedResources)).toBe(true)
    expect(reflector.get(IS_PUBLIC_KEY, SiteRuntimeController.prototype.batchGetPublicViews)).toBe(true)
    expect(reflector.get(IS_PUBLIC_KEY, SiteRuntimeController.prototype.reportSyncResult)).toBe(true)
  ...
  it('uses Nest rawBody bytes for signature hashing when Express has already parsed body', async () => {
    ...
    await controller.getLatestPublishState(
      signedHeaders,
      { local_publish_version: 3 },
      { body: { local_publish_version: 3 }, rawBody } as never
    )

    expect(service.getLatestPublishState).toHaveBeenCalledWith(expect.objectContaining({
      rawBody
    }))
  })

  it('delegates changed resources, public views, sync result, and preview view to the service', async () => {
    ...
    await controller.listChangedResources(signedHeaders, { from_publish_version: 1 }, Buffer.from('{}'))
    await controller.batchGetPublicViews(signedHeaders, { resources: [] }, Buffer.from('{}'))
    await controller.reportSyncResult(signedHeaders, { local_publish_version: 8, status: 'completed' }, Buffer.from('{}'))
    ...
    expect(service.getPreviewView).toHaveBeenCalledWith({
      ...
      normalizedQuery: '',
      signedHeaders,
      body: { preview_token: 'token', resource_type: 'blog', resource_id: 'blog_a', locale: 'en-US' },
      rawBody: Buffer.from('{}')
    })
  })
})
