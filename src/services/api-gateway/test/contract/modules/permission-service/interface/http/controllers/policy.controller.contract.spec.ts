...
import { PolicyManagementGrpcAdapter } from '../../../../../../../src/modules/permission-service/adapters/policy-management-grpc.adapter';
import { PolicyController } from '../../../../../../../src/modules/permission-service/interface/http/controllers/policy.controller';
...
describe('PolicyController', () => {
    ...
    it('declares the expected permissions on readonly policy governance endpoints', () => {
        ...
        expect(reflector.get(REQUIRE_PERMISSIONS_METADATA_KEY, PolicyController.prototype.listPolicies)).toEqual(expect.objectContaining({ all: expect.any(Array) }));
        expect(reflector.get(REQUIRE_PERMISSIONS_METADATA_KEY, PolicyController.prototype.getPolicyById)).toEqual(expect.objectContaining({ all: expect.any(Array) }));
        expect(reflector.get(REQUIRE_PERMISSIONS_METADATA_KEY, PolicyController.prototype.listPoliciesByPermission)).toEqual(expect.objectContaining({ all: expect.any(Array) }));
    });
    ...
    it('forwards normalized readonly list filters to the proxy service', async () => {
        ...
        await expect(controller.listPolicies({
            isEnabled: false,
            keyword: 'deny',
            page: 2,
            pageSize: 50,
            ...
            tenantId: 'tenant-1'
        } as any, source as any)).resolves.toEqual({
            page: 2,
            pageSize: 50,
            policies: [],
            total: 0
    ...
    it('forwards policy detail and permission-linked policy queries to the proxy service', async () => {
        ...
        await expect(controller.listPoliciesByPermission('permission.role_instance.update', { tenantId: 'tenant-1' } as any, source as any)).resolves.toEqual({ policies: [] });
