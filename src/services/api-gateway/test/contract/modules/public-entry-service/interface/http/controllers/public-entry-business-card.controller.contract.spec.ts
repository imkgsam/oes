...
import { PublicEntryBusinessCardService } from '../../../../../../../src/modules/public-entry-service/public-entry-business-card.service'
import { PublicEntryBusinessCardController } from '../../../../../../../src/modules/public-entry-service/interface/http/controllers/public-entry-business-card.controller'
...
describe('PublicEntryBusinessCardController', () => {
  ...
  it('declares admin permission metadata while keeping self-view RBAC-free and public render anonymous', () => {
    ...
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.ensurePrimaryCard
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.MANAGE] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.listCards
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.READ] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.updateConfig
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.MANAGE] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.enableCard
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.ENABLE] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.disableCard
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.DISABLE] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.bindPublicEntry
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.PUBLIC_ENTRY_MANAGE] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.getVisitSummary
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.STATS_READ] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.listContactAssetCandidates
      )
    ).toEqual({ all: [PUBLIC_ENTRY_BUSINESS_CARD_PERMISSION_CODES.READ] })
    expect(
      reflector.get(
        REQUIRE_PERMISSIONS_METADATA_KEY,
        PublicEntryBusinessCardController.prototype.getOwnPreview
      )
    ).toBeUndefined()
    expect(
  ...
  it('exposes contract-aligned public render and vCard paths with a compatibility vCard alias', () => {
    expect(
      Reflect.getMetadata(
        PATH_METADATA,
        PublicEntryBusinessCardController.prototype.renderPublicCard
      )
    ).toBe('public-entry/public/business-cards/:businessCardId')
    expect(
      Reflect.getMetadata(PATH_METADATA, PublicEntryBusinessCardController.prototype.downloadVCard)
    ).toEqual([
  ...
  it('registers the suffix vCard alias before the generic public render route', async () => {
    service.generateVCard.mockResolvedValue({
    ...
    try {
      const moduleRef = await Test.createTestingModule({
        controllers: [PublicEntryBusinessCardController],
        providers: [{ provide: PublicEntryBusinessCardService, useValue: service }]
      }).compile()
      app = moduleRef.createNestApplication()
      await app.init()

      const vcardResponse = await request(app.getHttpServer())
        .get('/public-entry/public/business-cards/card-1.vcf')
        .set('x-request-id', 'req-vcard')
        .expect(200)
        .expect('Content-Type', /text\/vcard/)
        .expect('Content-Disposition', 'attachment; filename="business-card-card-1.vcf"')
      const renderResponse = await request(app.getHttpServer())
        .get('/public-entry/public/business-cards/card-1')
        .set('x-request-id', 'req-render')
        .expect(200)

      expect(vcardResponse.text).toBe('BEGIN:VCARD\r\nEND:VCARD\r\n')
      expect(renderResponse.body).toEqual({ state: 'AVAILABLE' })
      expect(service.generateVCard).toHaveBeenCalledWith(
        'card-1',
        expect.objectContaining({ requestId: 'req-vcard' })
      )
      expect(service.renderPublicCard).toHaveBeenCalledWith(
        'card-1',
        expect.objectContaining({ requestId: 'req-render' })
      )
    } finally {
      await app?.close()
    }
  ...
  it('delegates tenant admin, employee self-view, public render, and vCard endpoints to the BFF service', async () => {
    ...
    await controller.getVisitSummary(
      'tenant-1',
      'card-1',
      { from: '2026-06-08T00:00:00.000Z' },
      source as never
    )
    await controller.listContactAssetCandidates(
      'tenant-1',
      { employeeId: 'emp-1' },
      source as never
    )
    ...
    expect(service.ensurePrimaryCard).toHaveBeenCalledWith(
      'tenant-1',
      { employeeId: 'emp-1' },
      source
    )
    ...
    expect(service.listContactAssetCandidates).toHaveBeenCalledWith(
      'tenant-1',
      { employeeId: 'emp-1' },
      source
    )
