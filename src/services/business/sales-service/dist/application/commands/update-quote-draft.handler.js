"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateQuoteDraftHandler = void 0;
const common_1 = require("@nestjs/common");
const cqrs_1 = require("@nestjs/cqrs");
const exceptions_1 = require("@oes/common/exceptions");
const tokens_1 = require("../../common/constants/tokens");
const sales_errors_1 = require("../../common/errors/sales.errors");
const sales_records_1 = require("../../domain/models/sales-records");
const sales_assertions_1 = require("../support/sales-assertions");
const sales_line_builders_1 = require("../support/sales-line-builders");
const update_quote_draft_command_1 = require("./update-quote-draft.command");
/** UpdateQuoteDraftHandler replaces the current mutable quote draft without generating any published version. */
let UpdateQuoteDraftHandler = class UpdateQuoteDraftHandler {
    constructor(quoteRepository) {
        this.quoteRepository = quoteRepository;
    }
    async execute(command) {
        (0, sales_assertions_1.assertRequiredString)(command.tenantId, 'tenantId');
        (0, sales_assertions_1.assertRequiredString)(command.quoteId, 'quoteId');
        (0, sales_assertions_1.assertRequiredString)(command.draftMutation.customerTenantPartyId, 'draftMutation.customerTenantPartyId');
        const existing = await this.quoteRepository.findById(command.tenantId, command.quoteId);
        if (!existing) {
            throw exceptions_1.ExceptionFactory.domain(sales_errors_1.SALES_NOT_FOUND, {
                quoteId: command.quoteId
            });
        }
        const updated = {
            ...existing,
            customerTenantPartyId: command.draftMutation.customerTenantPartyId,
            opportunityRef: command.draftMutation.opportunityRef ?? null,
            status: existing.latestPublishedVersionId ? sales_records_1.SalesQuoteStatus.PUBLISHED : sales_records_1.SalesQuoteStatus.DRAFT,
            lines: (0, sales_line_builders_1.toQuoteLineRecords)(command.draftMutation.lines ?? [])
        };
        return this.quoteRepository.save(updated);
    }
};
exports.UpdateQuoteDraftHandler = UpdateQuoteDraftHandler;
exports.UpdateQuoteDraftHandler = UpdateQuoteDraftHandler = __decorate([
    (0, common_1.Injectable)(),
    (0, cqrs_1.CommandHandler)(update_quote_draft_command_1.UpdateQuoteDraftCommand),
    __param(0, (0, common_1.Inject)(tokens_1.TOKENS.QUOTE_REPOSITORY)),
    __metadata("design:paramtypes", [Object])
], UpdateQuoteDraftHandler);
//# sourceMappingURL=update-quote-draft.handler.js.map