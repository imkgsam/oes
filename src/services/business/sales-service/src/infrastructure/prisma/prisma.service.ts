...
/** PrismaService manages the sales-service database connection and ambient transaction reuse. */
@Injectable()
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.error('[SALES_SERVICE] PrismaService connection failed', exception)
      process.exit(1)
    }
  }
