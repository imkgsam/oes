...
/** PrismaService manages the wms-service database connection and ambient transaction reuse. */
let PrismaService = class PrismaService extends prisma_1.PrismaClient {
    logger = new common_1.Logger('PrismaService');
    transactionStorage = new async_hooks_1.AsyncLocalStorage();
    ...
    async onModuleInit() {
        try {
            ...
            this.logger.error(`[WMS_SERVICE] PrismaService raw connection error: ${error instanceof Error ? `${error.name}: ${error.message}` : String(error)}`);
            this.logger.error('[WMS_SERVICE] PrismaService connection failed', exception);
            process.exit(1);
        }
    }
