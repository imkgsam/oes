...
function createWmsServices(client) {
  ...
  return {
    ...
    receipt: {
      management: {
        createReceiptDraft: async (request) => firstValueFrom(receiptManagement.createReceiptDraft(request)),
...
async function main() {
  ...
  try {
    ...
    const result = await runWmsSmokeFlow(services, seed, (message) => console.log(`[wms-smoke] ${message}`));
...
await main().catch((error) => {
  throw new Error(error instanceof Error ? `wms-service smoke failed: ${error.message}` : String(error));
});
