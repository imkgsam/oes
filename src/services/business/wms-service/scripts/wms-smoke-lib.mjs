...
/** createSmokeSeed builds deterministic business fixtures without transport authority. */
...
export async function runWmsSmokeFlow(services, seed, report = () => undefined) {
  ...
  const seededWarehouse = warehouse.warehouses.find(
    (item) => item?.warehouseId === seed.warehouseId
  )
  if (!seededWarehouse) {
    throw new Error(
      'wms-service smoke failed: ListWarehouses did not return the seeded warehouse fixture'
    )
  }
  ...
  if ((linedReceipt.lineCount ?? 0) !== 1) {
    throw new Error(
      'wms-service smoke failed: AddOrReplaceReceiptLines did not persist exactly one receipt line'
    )
  }
  ...
  if (
    !Array.isArray(postResponse.postedStockLedgerEntryIds) ||
    postResponse.postedStockLedgerEntryIds.length !== 1
  ) {
    throw new Error(
      'wms-service smoke failed: PostReceipt did not return exactly one posted ledger entry id'
    )
  }
  ...
  if (!postedEntry) {
    throw new Error(
      'wms-service smoke failed: SearchStockLedgerEntries did not return the posted receipt ledger row'
    )
  }
  ...
  if (!projectedBalance) {
    throw new Error(
      'wms-service smoke failed: SearchInventoryBalances did not return the posted inventory projection'
    )
  }
...
function assertWmsServices(services) {
  if (
    ...
    throw new Error(
      'wms-service smoke failed: warehouse, receipt, or inventory clients are not fully configured'
    )
  }
}
