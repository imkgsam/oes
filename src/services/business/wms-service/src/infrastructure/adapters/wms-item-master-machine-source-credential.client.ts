...
/** Obtains WMS's deployment-owned MACHINE source credential from Auth. */
export class WmsItemMasterMachineSourceCredentialClient {
  ...
  async issue(): Promise<string> {
    const result = await safeGrpcCall(
      this.getService().issueMachineWorkloadSourceCredential(
        {
          machinePrincipalId: required('WMS_ITEM_MASTER_MACHINE_PRINCIPAL_ID'),
          machineWorkloadBindingId: required('WMS_ITEM_MASTER_MACHINE_WORKLOAD_BINDING_ID'),
          machineWorkloadBindingVersion: required(
            'WMS_ITEM_MASTER_MACHINE_WORKLOAD_BINDING_VERSION'
          )
        },
        new Metadata()
      ),
      { caller: 'wms-service', method: 'IssueMachineWorkloadSourceCredential' }
    )
...
/** Resolves the Auth mTLS endpoint for WMS credential and token exchange calls. */
