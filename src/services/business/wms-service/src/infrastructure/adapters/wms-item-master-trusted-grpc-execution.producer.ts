...
import { WmsItemMasterExecutionTokenExchangeClient } from './wms-item-master-execution-token-exchange.client'
import { WmsItemMasterMachineSourceCredentialProvider } from './wms-item-master-machine-source-credential.provider'

const ITEM_MASTER_AUDIENCE = 'urn:oes:service:item-master-service'

/** Produces exact INTERNAL Item Master metadata from WMS's SYSTEM MACHINE identity. */
export class WmsItemMasterTrustedGrpcExecutionProducer {
  ...
  constructor(
    private readonly source: WmsItemMasterMachineSourceCredentialProvider,
    private readonly exchange: WmsItemMasterExecutionTokenExchangeClient
  ) {}
  ...
  async createMetadata(
    ...
    const subject = required('WMS_ITEM_MASTER_MACHINE_PRINCIPAL_ID')
    if (
      !tenantId ||
...
/** Requires WMS's deployment-owned trust facts and fails before transport on gaps. */
