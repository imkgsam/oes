...
/** Exchanges WMS's verified HUMAN current-hop credential for a Procurement target token. */
export class WmsProcurementExecutionTokenExchangeClient implements ExecutionTokenExchangeClient {
  ...
  async exchange(
    ...
    const result = await safeGrpcCall(
      ...
      { caller: 'wms-service', method: 'ExchangeExecutionToken' }
    )
  ...
  /** Lazily resolves Auth's mTLS-bound execution-token exchange client. */
