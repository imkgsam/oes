...
import {
  AddOrReplaceReceiptLinesCommand
} from '../../application/commands/add-or-replace-receipt-lines.command'
...
export class WmsManagementGrpcController implements ReceiptManagementServiceController {
  ...
  async createReceiptDraft(request: CreateReceiptDraftRequest): Promise<CreateReceiptDraftResponse> {
  ...
  async cancelReceiptDraft(request: CancelReceiptDraftRequest): Promise<CancelReceiptDraftResponse> {
    const context = WmsRpcContextValidator.assertManagementContext(request)
    return this.runWithContext(context, () =>
      this.auditService.recordCommand(
        {
...
function toDomainRestrictedStatusReasonCode(value?: ProtoRestrictedStatusReasonCode): RestrictedStatusReasonCode {
  switch (value) {
...
function toDomainReceiptTrackingRefType(value?: ProtoReceiptTrackingRefType): ReceiptTrackingRefType {
  switch (value) {
...
function buildDownstreamRequestContext(context: {
  ...
  return {
    internalServiceName: SERVICE_NAMES.WMS,
