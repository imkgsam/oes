...
import { getRpcAuthorizationModeDeclaration, WMS_MANAGEMENT_PERMISSION_CODES } from '@oes/common/authorization';
...
import { WMS_AUDIENCE, WmsTrustedBusinessExecutionGuard } from '../../src/modules/wms-trusted-execution.module';
const queryCodes = {
...
describe('WMS trusted gRPC security matrix Contract', () => {
    it('declares all 15 RPCs as exact HUMAN/WEB BUSINESS methods using five Codes', () => {
        const entries = [
            ...Object.entries(queryCodes).map((entry) => [WmsQueryGrpcController.prototype, ...entry] as const),
        ...
        expect(new Set(entries.map((entry) => entry[2]))).toEqual(new Set([
            'wms.warehouse.read',
            'wms.location.read',
            'wms.receipt.read',
            'wms.receipt.manage',
    ...
    it('installs token-only guard and context validator on both controller classes', () => {
        for (const controller of [WmsQueryGrpcController, WmsManagementGrpcController]) {
            expect(Reflect.getMetadata('__guards__', controller)).toEqual(expect.arrayContaining([WmsTrustedBusinessExecutionGuard, WmsRpcContextValidator]));
        }
    });
...
async function runBusiness(overrides: Record<string, unknown> = {}, body: Record<string, unknown> = {}, includeBearer = true) {
    ...
    const context = executionContext(WmsQueryGrpcController, WmsQueryGrpcController.prototype.getWarehouse, body, metadata, workloadIdentity);
    await new WmsTrustedBusinessExecutionGuard(new Reflector(), strictVerifier(verified) as never, { getVerifiedWorkloadIdentity: jest.fn().mockResolvedValue(workloadIdentity) } as never, WMS_AUDIENCE).canActivate(context);
    return { context, body };
}
