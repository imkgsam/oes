...
export function createSmokeSeed(now = Date.now()) {
  ...
  return {
    ...
    tags: (process.env.SRM_SMOKE_TAGS || 'smoke,srm')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean),
    partyCanonicalName:
      process.env.SRM_SMOKE_PARTY_CANONICAL_NAME || `SRM Smoke Organization ${suffix}`,
    partyLocalDisplayName:
      process.env.SRM_SMOKE_PARTY_LOCAL_DISPLAY_NAME || `SRM Smoke Party ${suffix}`,
...
export async function runSrmSmokeFlow(services, seed, log = () => {}) {
  ...
  if (beforeCreatePage.total !== 0 || beforeCreatePage.suppliers.length !== 0) {
    throw new Error(
      'srm-service smoke failed: SearchSuppliers should return an empty page for a fresh tenant'
    )
  }

  log(`supplier search empty before create for tenant=${seed.tenantId}`)

  const createResponse = await services.srm.management.createSupplierProfile(
    createCreateRequest(seed)
  )
  const createdSupplier = createResponse?.supplier
  if (!createdSupplier?.supplierId || !createdSupplier?.displayName) {
    throw new Error(
      'srm-service smoke failed: CreateSupplierProfile did not return a persisted supplier profile'
    )
  }
  ...
  const searchedSupplier = afterCreatePage.suppliers.find(
    (supplier) => supplier?.supplierId === createdSupplier.supplierId
  )
  if (!searchedSupplier) {
  ...
  if (bindingTenantPartyId !== tenantPartyId) {
    throw new Error(
      'srm-service smoke failed: BindSupplierToTenantParty did not return the expected tenantParty binding'
    )
  }
  ...
  if (activationResponse?.supplier?.status !== 1) {
    throw new Error(
      'srm-service smoke failed: ChangeSupplierStatus did not activate the bound supplier'
    )
  }
  ...
  if (
    !itemManagement?.createItemModel ||
    !itemManagement?.createItem ||
    !itemManagement?.setItemCapabilities
  ) {
    return {
      supplierId: createdSupplier.supplierId,
      supplierNo: createdSupplier.supplierNo ?? null,
  ...
  if (
    !createdOffering?.supplierOfferingId ||
    createdOffering.itemId !== createdItemId ||
    createdOffering.status !== 1
  ) {
    throw new Error(
      'srm-service smoke failed: UpsertSupplierOffering did not return the expected active offering'
    )
  }
