...
import { createGatewayItemManagementClient } from '../../../../../scripts/local/item-master-smoke-fixture.mjs'
...
const { SUPPLIER_QUERY_SERVICE_NAME, SUPPLIER_MANAGEMENT_SERVICE_NAME } = require(
...
function createSrmGrpcClient() {
  const host = normalizeGrpcClientHost(
    process.env.SRM_SERVICE_GRPC_HOST || process.env.GRPC_LISTEN_HOST || '127.0.0.1'
  )
...
async function main() {
  ...
  try {
    ...
    if ((process.env.SRM_SMOKE_ENABLE_OFFERING ?? 'true').toLowerCase() !== 'false') {
      const services = createGatewayItemManagementClient(seed)
      itemMasterServices = {
        management: {
