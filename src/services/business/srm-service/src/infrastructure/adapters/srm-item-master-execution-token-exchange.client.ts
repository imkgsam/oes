...
/** Exchanges SRM's private source credential for one target-bound Item Master token. */
export class SrmItemMasterExecutionTokenExchangeClient implements ExecutionTokenExchangeClient {
  ...
  async exchange(
    ...
    const result = await safeGrpcCall(
      ...
      { caller: 'srm-service', method: 'ExchangeExecutionToken' }
    )
