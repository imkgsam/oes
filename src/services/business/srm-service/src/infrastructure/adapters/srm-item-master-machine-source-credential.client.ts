...
/** Obtains SRM's deployment-owned MACHINE source credential from Auth. */
export class SrmItemMasterMachineSourceCredentialClient {
  ...
  async issue(): Promise<string> {
    const result = await safeGrpcCall(
      this.getService().issueMachineWorkloadSourceCredential(
        {
          machinePrincipalId: required('SRM_ITEM_MASTER_MACHINE_PRINCIPAL_ID'),
          machineWorkloadBindingId: required('SRM_ITEM_MASTER_MACHINE_WORKLOAD_BINDING_ID'),
          machineWorkloadBindingVersion: required(
            'SRM_ITEM_MASTER_MACHINE_WORKLOAD_BINDING_VERSION'
          )
        },
        new Metadata()
      ),
      { caller: 'srm-service', method: 'IssueMachineWorkloadSourceCredential' }
    )
...
/** Resolves the Auth mTLS endpoint for SRM credential and token exchange calls. */
