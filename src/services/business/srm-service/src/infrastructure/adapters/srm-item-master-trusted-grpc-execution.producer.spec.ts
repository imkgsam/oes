...
/** Verifies SRM's target-profile producer maps missing deployment roots to the Item Master error family. */
describe('SRM Item Master producer', () => {
  it('fails closed before exchange when deployment trust is unavailable', async () => {
    ...
    process.env.SRM_ITEM_MASTER_MACHINE_PRINCIPAL_ID = 'machine-srm'
    await expect(
      new SrmItemMasterTrustedGrpcExecutionProducer({} as never, {} as never).createMetadata(
