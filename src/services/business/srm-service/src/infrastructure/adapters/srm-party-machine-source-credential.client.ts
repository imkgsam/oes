...
export class SrmPartyMachineSourceCredentialClient {
  private client?: ClientGrpc
  private service?: MachineWorkloadSourceCredentialServiceClient

  async issue(): Promise<string> {
    const machinePrincipalId = required('SRM_PARTY_MACHINE_PRINCIPAL_ID')
    const machineWorkloadBindingId = required('SRM_PARTY_MACHINE_WORKLOAD_BINDING_ID')
    const machineWorkloadBindingVersion = required('SRM_PARTY_MACHINE_WORKLOAD_BINDING_VERSION')
