...
/** Exchanges MES's private source credential for one target-bound Item Master token. */
export class MesItemMasterExecutionTokenExchangeClient implements ExecutionTokenExchangeClient {
  ...
  async exchange(
    ...
    const result = await safeGrpcCall(
      ...
      { caller: 'mes-service', method: 'ExchangeExecutionToken' }
    )
