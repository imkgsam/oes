...
/** PrismaService manages the mes-service database connection and ambient transaction reuse. */
@Injectable()
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger('MesPrismaService')
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.error(
        `[MES_SERVICE] PrismaService raw connection error: ${
          error instanceof Error ? `${error.name}: ${error.message}` : String(error)
        }`
      )
      this.logger.error('[MES_SERVICE] PrismaService connection failed', exception)
      process.exit(1)
    }
  }
