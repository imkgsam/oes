...
/** commandContext builds the shared MES command context required by mold Integration calls. */
function commandContext(prefix: string, commandId: string) {
  return {
    ...
    auditContext: {
      ...
      reason: 'Integration integration test',
      source: 'jest'
    },
    commandId
  }
...
/** queryContext builds the shared MES query context required by mold Integration calls. */
...
/** itemModelRef builds the required MoldDesign primary ItemModel snapshot for Integration fixtures. */
...
describe('Prisma MES mold repository Integration', () => {
  ...
  it('life counter / should adjust counters through direct id lookup instead of paginated scans', async () => {
    ...
    const design = await management.registerMoldDesign({
      ...
      name: 'Integration Counter Mold Design',
      primaryItemModelRef: itemModelRef(prefix),
