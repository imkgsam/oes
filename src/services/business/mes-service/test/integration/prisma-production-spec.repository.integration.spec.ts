...
/** StubManufacturableItemLookupPort lets Prisma Integration tests validate MES rules without calling item-master-service. */
...
/** commandContext builds the shared MES command context required by ProductionSpec Integration calls. */
function commandContext(prefix: string, commandId: string) {
  return {
    ...
    auditContext: {
      ...
      reason: 'Integration production spec test',
      source: 'jest'
    },
    commandId
  }
...
/** queryContext builds the shared MES query context required by ProductionSpec Integration calls. */
...
function itemModelRef(prefix: string) {
  return {
    ...
    modelNameSnapshot: 'Integration Item Model'
  }
}

describe('Prisma ProductionSpec repository Integration', () => {
  ...
  it('service transaction / should persist lifecycle updates, audit, outbox, and query filters', async () => {
    const created = await management.createProductionSpec({
      ...
      name: 'Integration Wash Basin A100 High Pressure',
      revisionCode: 'R1',
      itemRef: {
        itemId: `${prefix}_item`
      }
