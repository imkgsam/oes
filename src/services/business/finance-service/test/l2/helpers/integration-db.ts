...
/** ensureIntegrationDatabaseUrl loads finance-service DATABASE_URL for Prisma-backed L2 tests. */
...
export async function createPrismaForIntegration(): Promise<PrismaService> {
  ...
  try {
    ...
    throw new Error(
      `finance-service L2 tests require a reachable PostgreSQL database. Current DATABASE_URL target: ${safeTarget}. Cause: ${
...
export function createTestPrefix(): string {
  return `finance_l2_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
}
