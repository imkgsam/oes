...
import { AsyncLocalTrustedExecutionContextAccessor, CertificateBoundExecutionTokenCache, CRM_INTERNAL_PERMISSION_CODES, CRM_MANAGEMENT_PERMISSION_CODES, ExecutionTokenJwksCache, ExecutionTokenVerifier, getAuthenticatedGrpcRequestContext, getPermissionCodeDefinition, getRpcAuthorizationModeDeclaration, inboundExecutionTokenCredentialScope, permissionDefinitionFingerprint, TrustedExecutionRegistry, TrustedGrpcMetadataProvider } from '@oes/common/authorization';
import { AuthorizationPrincipalTypeProto, AuthorizationScopeLevelProto } from '@oes/common/generated/permission_service';
...
import { CRM_AUDIENCE, CrmTrustedBusinessExecutionGuard, CrmTrustedInternalExecutionGuard } from '../../src/modules/crm-trusted-execution.module';
...
import { CompositeSourceCredentialVerifier, PermissionDecisionGrpcResolver } from '../../../../system/auth-service/src/modules/token/execution-token.module';
...
describe('CRM trusted gRPC security matrix Contract', () => {
    it('declares exact 14 BUSINESS plus one INTERNAL method with five dual-terminal methods', () => {
        const business = [
            ...Object.entries(queryCodes).map(([method, code]) => [CustomerQueryGrpcController.prototype, method, code] as const),
            ...Object.entries(managementCodes).map(([method, code]) => [CustomerManagementGrpcController.prototype, method, code] as const)
        ];
        ...
        expect(getRpcAuthorizationModeDeclaration(CrmObjectReferenceGrpcController.prototype, 'validateCrmObjectReference')).toEqual({
            mode: 'INTERNAL',
    ...
    it('installs exact guards/context validator and deployment mTLS on all controllers', () => {
        for (const controller of [CustomerQueryGrpcController, CustomerManagementGrpcController]) {
            expect(Reflect.getMetadata('__guards__', controller)).toEqual(expect.arrayContaining([CrmTrustedBusinessExecutionGuard, CustomerRpcContextValidator]));
        }
        expect(Reflect.getMetadata('__guards__', CrmObjectReferenceGrpcController)).toEqual(expect.arrayContaining([CrmTrustedInternalExecutionGuard, CustomerRpcContextValidator]));
    });
    it('admits WEB and BROWSER_EXTENSION only on the exact five-method set', async () => {
        await expect(runBusiness(CustomerQueryGrpcController, CustomerQueryGrpcController.prototype.getCrmAccount, queryCodes.getCrmAccount, { sessionTerminal: 'BROWSER_EXTENSION' })).resolves.toBeDefined();
        await expect(runBusiness(CustomerQueryGrpcController, CustomerQueryGrpcController.prototype.listCrmAccounts, queryCodes.listCrmAccounts, { sessionTerminal: 'BROWSER_EXTENSION' })).rejects.toThrow();
    });
    it.each([
        ...
        await expect(runBusiness(CustomerQueryGrpcController, CustomerQueryGrpcController.prototype.listCrmAccounts, queryCodes.listCrmAccounts, overrides)).rejects.toThrow();
    });
    ...
    it('runs the DESIGN_GAP expected-failure lane for Collaboration OBO Permission scope', async () => {
        ...
        const signingKey = {
            ...
            retireAfterUnixSeconds: now + 3600
        };
        const signer = {
            currentSigningKey: async () => signingKey,
            publishedKeys: async () => [signingKey],
            sign: async (_kid: string, input: Uint8Array) => sign('sha256', input, { key: pair.privateKey, dsaEncoding: 'ieee-p1363' })
        };
        const createVerifier = (audience: string, workload: string) => new ExecutionTokenVerifier({
        ...
        const subjectHeader = Buffer.from(JSON.stringify({ alg: 'ES256', typ: 'at+jwt', kid: signingKey.kid })).toString('base64url');
        const subjectClaims = Buffer.from(JSON.stringify({
            iss: issuer,
        ...
        const permissionHandler = new ResolveWorkloadIssuanceHandler(workloadRepository as never, permissionRepository as never, new PermissionDecisionPolicy(), permissionAudit as never);
        const permissionDecision = jest.fn((permissionRequest) => from(permissionHandler.execute(new ResolveWorkloadIssuanceQuery({
        ...
        const exchange = new ExecutionTokenExchangeService(authRegistry, signer as never, () => now, audit);
        const permission = new PermissionDecisionGrpcResolver({ getService: () => ({ resolveWorkloadIssuance: permissionDecision }) } as never, exchange, { spiffeId: 'spiffe://oes/auth-service', certificateThumbprint: 'A'.repeat(43) }, 'policy-v1');
        ...
        }, new CompositeSourceCredentialVerifier({} as never, {} as never, new ExecutionTokenSubjectCredentialVerifier(signer as never, identity as never, authRegistry, () => now)), permission);
        ...
        expect(exchangeFailure).toEqual(new Error('execution token exchange lacks an authoritative Permission decision'));
        expect(permissionAudit.emitIssuanceDecision).toHaveBeenCalledWith(expect.objectContaining({
            allowed: false,
    ...
    it('rejects missing credential/correlation and all retired body authority', async () => {
        ...
        for (const body of [
            ...
            const result = await runBusiness(CustomerQueryGrpcController, CustomerQueryGrpcController.prototype.listCrmAccounts, queryCodes.listCrmAccounts, {}, body);
...
async function runBusiness(controller: object, handler: unknown, code: string, overrides: Record<string, unknown> = {}, body: Record<string, unknown> = {}) {
    ...
    await new CrmTrustedBusinessExecutionGuard(new Reflector(), strictVerifier(verified) as never, { getVerifiedWorkloadIdentity: jest.fn().mockResolvedValue(workloadIdentity) } as never, CRM_AUDIENCE).canActivate(context);
    return { context, body };
}
/** Executes CRM's INTERNAL guard with request-private HUMAN_OBO facts. */
async function runInternal(overrides: Record<string, unknown> = {}, body: Record<string, unknown> = {}, includeBearer = true, includeCorrelation = true) {
    ...
    const context = executionContext(CrmObjectReferenceGrpcController, CrmObjectReferenceGrpcController.prototype.validateCrmObjectReference, body, metadata, workloadIdentity);
    await new CrmTrustedInternalExecutionGuard(new Reflector(), strictVerifier(verified) as never, { getVerifiedWorkloadIdentity: jest.fn().mockResolvedValue(workloadIdentity) } as never, CRM_AUDIENCE).canActivate(context);
    return { context, body };
}
