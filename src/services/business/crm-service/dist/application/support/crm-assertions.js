...
exports.assertKnownCustomerStatus = assertKnownCustomerStatus;
...
function assertRequiredString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw exceptions_1.ExceptionFactory.application(crm_errors_1.CRM_INVALID_ARGUMENT, { field });
    }
}
...
function assertOperatorContext(value) {
    if (!value) {
        throw exceptions_1.ExceptionFactory.application(crm_errors_1.CRM_UNAUTHENTICATED, {
...
function assertKnownCustomerStatus(status) {
    if (status !== crm_records_1.CustomerStatus.ACTIVE_CUSTOMER &&
        status !== crm_records_1.CustomerStatus.BLOCKED &&
        status !== crm_records_1.CustomerStatus.ARCHIVED) {
        throw exceptions_1.ExceptionFactory.application(crm_errors_1.CRM_INVALID_ARGUMENT, {
            field: 'targetStatus'
        });
    }
    return status;
...
//# sourceMappingURL=crm-assertions.js.map