...
/** PrismaService manages the procurement-service database connection and ambient transaction reuse. */
let PrismaService = class PrismaService extends prisma_1.PrismaClient {
    ...
    async onModuleInit() {
        try {
            ...
            this.logger.error(`[PROCUREMENT_SERVICE] PrismaService raw connection error: ${error instanceof Error ? `${error.name}: ${error.message}` : String(error)}`);
            this.logger.error('[PROCUREMENT_SERVICE] PrismaService connection failed', exception);
            process.exit(1);
        }
    }
