...
import {
  PROCUREMENT_INTERNAL_WORKLOAD_ALLOWLIST,
  ProcurementTrustedBusinessExecutionGuard,
  ProcurementTrustedInternalExecutionGuard
} from '../../src/modules/procurement-trusted-execution.module'
...
describe('Procurement trusted gRPC security matrix L3', () => {
  it('declares all 21 existing RPCs as exact HUMAN/WEB BUSINESS methods', () => {
    const entries = [
      ...Object.entries(queryCodes).map(
        (entry) => [ProcurementQueryGrpcController.prototype, ...entry] as const
      ),
      ...Object.entries(managementCodes).map(
        (entry) => [ProcurementManagementGrpcController.prototype, ...entry] as const
      )
    ]
  ...
  it('declares exactly one WMS HUMAN_OBO INTERNAL method', () => {
    ...
    ).toEqual({ mode: 'INTERNAL', permissions: { all: [internalCode] } })
  ...
  it('installs token-only guards on all three controller classes', () => {
    expect(Reflect.getMetadata('__guards__', ProcurementQueryGrpcController)).toEqual(
      expect.arrayContaining([
        ProcurementTrustedBusinessExecutionGuard,
        ProcurementRpcContextValidator
      ])
    )
    expect(Reflect.getMetadata('__guards__', ProcurementManagementGrpcController)).toEqual(
      expect.arrayContaining([
        ProcurementTrustedBusinessExecutionGuard,
        ProcurementRpcContextValidator
      ])
    )
    expect(Reflect.getMetadata('__guards__', ProcurementInternalQueryGrpcController)).toEqual(
      expect.arrayContaining([
        ProcurementTrustedInternalExecutionGuard,
        ProcurementRpcContextValidator
      ])
...
async function runBusiness(
  ...
  const context = executionContext(
    ...
    ProcurementQueryGrpcController.prototype.searchPurchaseRequests,
    body,
    metadata,
    workloadIdentity
  )
  await new ProcurementTrustedBusinessExecutionGuard(
    new Reflector(),
    verifier as never,
...
async function runInternal(overrides: Record<string, unknown> = {}) {
  ...
  const context = executionContext(
    ProcurementInternalQueryGrpcController,
    ProcurementInternalQueryGrpcController.prototype.resolveReceivingExpectationForReceipt,
    body,
    metadata,
    workloadIdentity
  )
  await new ProcurementTrustedInternalExecutionGuard(
    new Reflector(),
