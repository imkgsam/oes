...
/** Obtains PROCUREMENT's deployment-owned MACHINE source credential from Auth. */
export class ProcurementItemMasterMachineSourceCredentialClient {
  ...
  async issue(): Promise<string> {
    const result = await safeGrpcCall(
      this.getService().issueMachineWorkloadSourceCredential(
        {
          machinePrincipalId: required('PROCUREMENT_ITEM_MASTER_MACHINE_PRINCIPAL_ID'),
          machineWorkloadBindingId: required('PROCUREMENT_ITEM_MASTER_MACHINE_WORKLOAD_BINDING_ID'),
          machineWorkloadBindingVersion: required(
            'PROCUREMENT_ITEM_MASTER_MACHINE_WORKLOAD_BINDING_VERSION'
          )
        },
        new Metadata()
      ),
      { caller: 'procurement-service', method: 'IssueMachineWorkloadSourceCredential' }
    )
...
/** Resolves the Auth mTLS endpoint for PROCUREMENT credential and token exchange calls. */
