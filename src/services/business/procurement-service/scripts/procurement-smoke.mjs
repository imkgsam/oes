...
} = require(path.join(WORKSPACE_ROOT, 'src/common/dist/generated/procurement_service/procurement.js'));
const {
...
// createItemMasterGrpcClient binds one direct gRPC client to the configured downstream item-master-service endpoint for optional bootstrap.
...
function createProcurementServices(client) {
  ...
  return {
    query: {
      searchPurchaseRequests: async (request) => firstValueFrom(query.searchPurchaseRequests(request))
    },
    management: {
      createPurchaseRequest: async (request) => firstValueFrom(management.createPurchaseRequest(request)),
      submitPurchaseRequest: async (request) => firstValueFrom(management.submitPurchaseRequest(request)),
      decidePurchaseRequest: async (request) => firstValueFrom(management.decidePurchaseRequest(request)),
...
function createBootstrapHook(seed) {
  const shouldBootstrap = (process.env.PROCUREMENT_SMOKE_BOOTSTRAP_ACTIVE_OFFERING || 'true').toLowerCase() !== 'false';
  if (!shouldBootstrap) {
    return {};
  }
    ...
    ensureActiveOffering: async () => {
      ...
      try {
        const services = {
          ...
          itemMaster: createItemMasterServices(itemMasterClient, seed)
        };
...
function createItemMasterMetadata(seed) {
  ...
  metadata.set('x-internal-service-name', 'procurement-service');
...
function isOptionalBootstrapUnavailableError(error) {
  ...
  return (
    ...
    message.includes('14 ') ||
    message.includes('missing x-internal-service-name')
  );
}
