...
export function createSmokeSeed(now = Date.now()) {
  ...
  return {
    ...
    standardItemLine: {
      description:
        process.env.PROCUREMENT_SMOKE_STANDARD_DESCRIPTION ||
        `Procurement smoke standard demand ${suffix}`,
...
export async function runProcurementSmokeFlow(services, seed, log = () => {}) {
  ...
  const beforeCreate = await services.procurement.query.searchPurchaseRequests(
    createApprovedSearchRequest(seed)
  )
  const beforeCreatePage = assertPurchaseRequestPage(
    beforeCreate,
    'initial purchase request search'
  )
  ...
  log('purchase-request search returned the expected empty business page')
  ...
  if (createdDraft.status !== PURCHASE_REQUEST_STATUS_DRAFT) {
    throw new Error(
      'procurement-service smoke failed: CreatePurchaseRequest did not create a DRAFT purchase request'
    )
  }

  log(
    `created purchaseRequest=${createdDraft.purchaseRequestId} no=${createdDraft.requestNo ?? '(none)'}`
  )

  const submittedRequest = await requirePurchaseRequest(
    services.procurement.management.submitPurchaseRequest(
      createSubmitRequest(seed, createdDraft.purchaseRequestId)
    ),
    'SubmitPurchaseRequest'
  )
  if (submittedRequest.status !== PURCHASE_REQUEST_STATUS_SUBMITTED) {
    throw new Error(
      'procurement-service smoke failed: SubmitPurchaseRequest did not move the request to SUBMITTED'
    )
  }
  ...
  const approvedRequest = await requirePurchaseRequest(
    services.procurement.management.decidePurchaseRequest(
      createApproveRequest(seed, createdDraft.purchaseRequestId)
    ),
    'DecidePurchaseRequest'
  )
  if (approvedRequest.status !== PURCHASE_REQUEST_STATUS_APPROVED) {
    throw new Error(
      'procurement-service smoke failed: DecidePurchaseRequest did not approve the request'
    )
  }

  log(`approved purchaseRequest=${createdDraft.purchaseRequestId}`)

  const afterApprove = await services.procurement.query.searchPurchaseRequests(
    createApprovedSearchRequest(seed)
  )
  const afterApprovePage = assertPurchaseRequestPage(
    afterApprove,
    'post-approve purchase request search'
  )
  ...
  if (!searchedRequest) {
    throw new Error(
      'procurement-service smoke failed: approved request did not appear in SearchPurchaseRequests'
    )
  }
  ...
  const standardLine = Array.isArray(createdStandardDraft.lines)
    ? createdStandardDraft.lines[0]
    : null
  ...
  const purchaseOrder = await requirePurchaseOrder(
    services.procurement.management.convertPurchaseRequestToPurchaseOrder(
      createConvertRequest(
        seed,
        createdStandardDraft.purchaseRequestId,
        standardLine.purchaseRequestLineId,
        conversionTarget
      )
    ),
  ...
  log(
    `converted purchaseRequest=${createdStandardDraft.purchaseRequestId} purchaseOrder=${purchaseOrder.purchaseOrderId}`
  )
...
function assertProcurementServices(services) {
  ...
  if (
    ...
    throw new Error(
      'procurement-service smoke failed: procurement query/management clients are not fully configured'
    )
  }
}
...
async function requirePurchaseRequest(responsePromise, step) {
  ...
  if (!purchaseRequest?.purchaseRequestId) {
    throw new Error(
      `procurement-service smoke failed: ${step} did not return a purchaseRequest payload`
    )
  }
...
async function requirePurchaseOrder(responsePromise, step) {
  ...
  if (!purchaseOrder?.purchaseOrderId) {
    throw new Error(
      `procurement-service smoke failed: ${step} did not return a purchaseOrder payload`
    )
  }
...
function assertPurchaseRequestPage(response, step) {
  if (
    ...
    throw new Error(
      `procurement-service smoke failed: ${step} did not return the expected page payload`
    )
  }
