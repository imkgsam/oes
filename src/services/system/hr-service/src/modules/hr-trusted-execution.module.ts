...
@Injectable()
export class HrFoundationTrustedExecutionGuard extends TrustedExecutionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const allowed = token?.principalType === 'MACHINE'
      ...
      : token?.principalType === 'HUMAN' && token.sessionTerminal === 'WEB' && ['api-gateway', 'identity-service', 'tenant-org-service'].includes(workload)
...
@Module({
  providers: [
    HrPartyTrustedGrpcClient, HrPartyMachineSourceCredentialClient, HrPartyMachineSourceCredentialProvider, HrPartyExecutionTokenExchangeClient,
    ...
    { provide: HrFoundationTrustedExecutionGuard, useFactory: (reflector: Reflector, verifier: ExecutionTokenVerifier, identity: GrpcWorkloadIdentityProvider) => new HrFoundationTrustedExecutionGuard(reflector, verifier, identity), inject: [Reflector, ExecutionTokenVerifier, GrpcWorkloadIdentityProvider] }
  ],
  exports: [HrPartyTrustedGrpcClient, HrPartyTrustedGrpcExecutionProducer, ExecutionTokenVerifier, GrpcWorkloadIdentityProvider, HrFoundationTrustedExecutionGuard]
})
