...
const ERRORS = Object.freeze({
  CONTEXT_REQUIRED: 'HR_FOUNDATION_EXECUTION_CONTEXT_REQUIRED',
  FOUNDATION_UNAVAILABLE: 'HR_FOUNDATION_EXECUTION_UNAVAILABLE',
  SOURCE_CREDENTIAL_INVALID: 'HR_FOUNDATION_SOURCE_CREDENTIAL_INVALID'
})

/** Produces target-bound HUMAN_OBO metadata solely from the guard-retained inbound HUMAN credential. */
export class HrFoundationTrustedGrpcExecutionProducer {
  private readonly context = new AsyncLocalTrustedExecutionContextAccessor()
  private readonly exchange = new HrFoundationExecutionTokenExchangeClient()
  private readonly callers = new Map<string, InternalTrustedGrpcCaller>()

  async forBusinessCall(
    target: keyof typeof HR_FOUNDATION_TARGETS,
...
class HrFoundationExecutionTokenExchangeClient implements ExecutionTokenExchangeClient {
  ...
  async exchange(
    ...
    const response = await safeGrpcCall(
      ...
      { caller: 'hr-service', method: 'ExchangeExecutionToken' }
    )
...
/** Resolves one immutable Hr target profile and rejects caller-selected or wildcard audiences. */
export function requireHrFoundationTarget(
  target: keyof typeof HR_FOUNDATION_TARGETS
): HrFoundationTargetProfile {
  const profile = HR_FOUNDATION_TARGETS[target]
  if (!profile || profile.audience.includes('*')) {
    throw new Error('Hr foundation target is not registered')
  }
  return profile
}
