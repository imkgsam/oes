import { HR_FOUNDATION_TARGETS, requireHrFoundationTarget } from '../../../../src/infrastructure/adapters/foundation-trusted-grpc.clients'
...
describe('Hr foundation trusted gRPC targets', () => {
  it('contains only the frozen target set', () => {
    expect(Object.keys(HR_FOUNDATION_TARGETS)).toEqual([
      'auth-service',
      'identity-service',
      'permission-service',
      'tenant-org-service'
    ])
    for (const target of Object.keys(HR_FOUNDATION_TARGETS) as Array<
      keyof typeof HR_FOUNDATION_TARGETS
    >) {
