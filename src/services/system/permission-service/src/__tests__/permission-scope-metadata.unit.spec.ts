...
import { getPermissionCodeDefinition, permissionDefinitionFingerprint } from '@oes/common/authorization';
...
import { toPermissionResponse } from '../interfaces/grpc/permission-management.grpc.presenter';
...
describe('Permission scope metadata pipeline', () => {
    it('round-trips exact allowed scopes and definition fingerprint through the Prisma mapper', () => {
        ...
        const permission = new Permission('permission-id', definition.code, PermissionModule.PERMISSION_SERVICE, definition.description, PermissionKind.BUSINESS, false, [ScopeLevel.SYSTEM, ScopeLevel.TENANT], permissionDefinitionFingerprint(definition));
    ...
    ] as const)('preserves frozen eligibility metadata for %s', (code, kind, assignableTo, allowedScopeLevels) => {
