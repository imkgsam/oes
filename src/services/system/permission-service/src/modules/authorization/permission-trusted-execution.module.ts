...
const ALLOWED_CALLERS = new Set(['api-gateway', 'auth-service', 'hr-service', 'tenant-org-service', 'collaboration-service', 'public-entry-service'])

/** Enforces Permission BUSINESS/INTERNAL caller shape after Common verifies audience, Code, time and certificate binding. */
@Injectable()
export class PermissionFoundationTrustedExecutionGuard extends TrustedExecutionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!ALLOWED_CALLERS.has(workload)) throw new ForbiddenException('Permission caller workload is not permitted')
    if (token?.principalType === 'HUMAN' && token.sessionTerminal !== 'WEB') throw new ForbiddenException('Permission HUMAN execution requires WEB terminal')
    if (token?.principalType === 'MACHINE' && !['auth-service', 'public-entry-service'].includes(workload)) throw new ForbiddenException('Permission SYSTEM MACHINE caller is not permitted')
    return true
  }
}
...
@Module({
  providers: [
    ...
    { provide: GrpcWorkloadIdentityProvider, useFactory: () => runtime.workloadIdentityProvider },
    {
      ...
      useFactory: (reflector: Reflector, verifier: ExecutionTokenVerifier, identity: GrpcWorkloadIdentityProvider) => new PermissionFoundationTrustedExecutionGuard(reflector, verifier, identity),
      inject: [Reflector, ExecutionTokenVerifier, GrpcWorkloadIdentityProvider]
    },
    {
      provide: TrustedInternalExecutionGuard,
      useFactory: (reflector: Reflector, verifier: ExecutionTokenVerifier, identity: GrpcWorkloadIdentityProvider) => new TrustedInternalExecutionGuard(reflector, verifier, identity, PERMISSION_AUDIENCE),
      inject: [Reflector, ExecutionTokenVerifier, GrpcWorkloadIdentityProvider]
    }
  ],
...
function readWorkloadName(spiffeId: string): string {
  try { const value = new URL(spiffeId); return value.protocol === 'spiffe:' ? value.pathname.split('/').filter(Boolean).at(-1) ?? '' : '' } catch { return '' }
}
