...
| 模块 | 功能项 | 说明 | 优先级 | 状态 | 备注 |
|---|---|---|---|---|---|
| 角色管理 | 查看角色详情 | 按 ID 查询角色 | P0 | 已实现 | |
| 角色管理 | 查看角色列表 | 支持基础列表查询 | P0 | 已实现 | 需补分页/过滤 |
| 角色管理 | 创建角色 | 创建租户角色或模板角色 | P0 | 已实现 | 需对模板/实例模型补充约束 |
| 角色管理 | 修改角色 | 修改名称、描述等可编辑字段 | P0 | 未开始 | |
| 角色管理 | 删除角色 | 删除前需校验引用关系 | P0 | 部分实现 | 已有删除命令，需补删除保护 |
| 角色管理 | 启用/停用角色 | 角色禁用后不再参与鉴权 | P0 | 未开始 | |
| 角色管理 | 查看角色持有的权限 | 区分直接绑定权限 | P0 | 未开始 | |
| 角色管理 | 查看持有该角色的账号 | 用于反查角色使用范围 | P0 | 未开始 | 已确认替代“角色继承”需求 |
| 角色管理 | 为角色添加权限 | 建立 `RolePermission` 关系 | P0 | 已实现 | |
| 角色管理 | 为角色移除权限 | 删除 `RolePermission` 关系 | P0 | 已实现 | |
