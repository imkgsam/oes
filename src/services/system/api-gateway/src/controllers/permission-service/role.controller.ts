...
export class RoleController {
  constructor(@InjectServiceClient('PERMI_TCP') private readonly permissionClient: ClientProxy) { }
  ...
  async findById(@Param('id') id: string) {
    const found = await firstValueFrom(this.permissionClient.send(PERMISSION_MESSAGES.GET_ROLE_BY_ID, { id }))
  ...
  async delete(@Param('id') id: string) {
    const deleted = await firstValueFrom(this.permissionClient.send(PERMISSION_MESSAGES.DELETE_ROLE, { id }))
