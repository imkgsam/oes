...
export class PermissionController {
  ...
  async getAllPermissions() {
    return safeRpcCall(
      this.permissionClient.send(PERMISSION_MESSAGES.LIST_PERMISSIONS, {})
    )
  }
  ...
  async createPermission(@Body() dto: CreatePermissionDto) {
    return safeRpcCall(
      this.permissionClient.send(PERMISSION_MESSAGES.CREATE_PERMISSION, dto)
    )
  }
  ...
  async findByCode(@Param('code') code: string) {
    return safeRpcCall(
      this.permissionClient.send(PERMISSION_MESSAGES.FIND_PERMISSION_BY_CODE, {
        code
      })
    )
  }
  ...
  async delete(@Param('id') id: string) {
    return safeRpcCall(
      this.permissionClient.send(PERMISSION_MESSAGES.DELETE_PERMISSION, { id })
    )
  }
}
