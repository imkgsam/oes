...
describe('SiteMediaLifecycleOperationWorker', () => {
  ...
  it('confirms only after provider acknowledgement', async () => {
    const repo = { claimDuePurgeOperations: jest.fn().mockResolvedValue([operation()]), confirmTakedownWithEvent: jest.fn(), schedulePurgeRetry: jest.fn() }
    ...
    expect(repo.confirmTakedownWithEvent).toHaveBeenCalledWith('op-1', 'cf-1', expect.any(Date)); expect(repo.schedulePurgeRetry).not.toHaveBeenCalled()
  })
  it('schedules bounded retry on provider failure', async () => {
    ...
    expect(repo.schedulePurgeRetry).toHaveBeenCalledWith('op-1', 1, expect.any(Date), 'PROVIDER_FAILURE'); expect(repo.confirmTakedownWithEvent).not.toHaveBeenCalled()
  })
