...
describe('GatewayExternalAccessTokenIssuer', () => {
  it('issues a distinct five-minute Gateway-only profile without internal claims', async () => {
    ...
    expect(header.typ).toBe('oes-external+jwt'); expect(claims).toMatchObject({ aud: 'api-gateway', sub: 'machine', tenant_id: 'tenant', credential_id: 'credential', authz_version: 'v1', scope: 'b.read', exp: 400 }); expect(claims.cnf).toBeUndefined(); expect(result).toMatchObject({ tokenType: 'Bearer', expiresAtUnixSeconds: '400', expiresInSeconds: '300', credentialId: 'credential', integrationMachineId: 'machine', tenantId: 'tenant' }); expect(result.accessToken.length).toBeLessThanOrEqual(4096)
  })
})
