...
export class MachineWorkloadSourceCredentialVerifier {
  ...
  async verify(sourceCredential: string, workload: VerifiedExecutionWorkload): Promise<TrustedExecutionContext> {
    ...
    if (header.typ !== 'oes-machine-source+jwt' || header.alg !== 'ES256' || typeof header.kid !== 'string' || claims.iss !== this.issuer || claims.aud !== 'urn:oes:service:auth-service' || claims.profile_version !== 1 || typeof claims.jti !== 'string' || typeof claims.sub !== 'string' || typeof claims.client_id !== 'string' || claims.client_id !== workload.spiffeId || cnf?.['x5t#S256'] !== workload.certificateThumbprint || typeof exp !== 'number' || exp <= this.now()) throw new Error(typeof exp === 'number' && exp <= this.now() ? 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_EXPIRED' : 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_INVALID')
