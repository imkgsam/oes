...
export class MachineWorkloadSourceCredentialVerifier {
  ...
  async verify(
    ...
    try {
      ...
      const context = await this.verifyUnchecked(sourceCredential, workload, correlation)
      await this.repository.recordVerificationOutcome({
        credentialId,
        eventType: 'MACHINE_SOURCE_CREDENTIAL_VERIFIED',
        reasonCode: '',
      ...
      const reasonCode =
        error instanceof Error && error.message.startsWith('EXECUTION_MACHINE_')
          ? error.message
          : 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_INVALID'
      await this.repository.recordVerificationOutcome({
        credentialId,
        eventType: 'MACHINE_SOURCE_CREDENTIAL_REJECTED',
        reasonCode,
        workloadSpiffeId: workload.spiffeId
      })
      throw error
  ...
  private async verifyUnchecked(
    ...
    if (!headerPart || !claimsPart || !signaturePart || extra)
      throw new Error('EXECUTION_MACHINE_SOURCE_CREDENTIAL_INVALID')
    ...
    const now = this.now()
    if (
      header.typ !== 'oes-machine-source+jwt' ||
      header.alg !== 'ES256' ||
      typeof header.kid !== 'string' ||
      claims.iss !== this.issuer ||
      claims.aud !== 'urn:oes:service:auth-service' ||
      claims.profile_version !== 1 ||
      ...
      throw new Error(
        isNumericDate(exp) && exp <= now
          ? 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_EXPIRED'
          : 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_INVALID'
      )
    const key = (await this.signer.publishedKeys()).find(
    ...
    if (!credential || credential.status === 'REVOKED')
      throw new Error('EXECUTION_MACHINE_SOURCE_CREDENTIAL_REVOKED')
    if (
      credential.status !== 'ACTIVE' ||
      credential.signingKid !== header.kid ||
      credential.profileVersion !== 1 ||
      Math.floor(credential.issuedAt.getTime() / 1_000) !== iat ||
      Math.floor(credential.expiresAt.getTime() / 1_000) !== exp ||
      credential.machinePrincipalId !== claims.sub ||
      credential.machineWorkloadBindingId !== claims.machine_workload_binding_id ||
      credential.machineWorkloadBindingVersion.toString() !==
        String(claims.machine_workload_binding_version) ||
      credential.workloadSpiffeId !== workload.spiffeId ||
      credential.certificateThumbprint !== workload.certificateThumbprint
    )
      throw new Error('EXECUTION_MACHINE_WORKLOAD_BINDING_MISMATCH')
    const decision = await this.identity.resolveMachinePrincipalForAuth({
      machinePrincipalId: credential.machinePrincipalId,
      bindingId: credential.machineWorkloadBindingId,
      bindingVersion: credential.machineWorkloadBindingVersion,
      workloadSpiffeId: workload.spiffeId,
      ...correlation
    })
    if (
      !decision.allowed ||
      decision.principalId !== credential.machinePrincipalId ||
      decision.principalType !== 'MACHINE' ||
      !isAllowedMachineType(decision.machineType) ||
      decision.principalLifecycleStatus !== 'ACTIVE' ||
      !decision.principalLifecycleVersion ||
      decision.bindingId !== credential.machineWorkloadBindingId ||
      decision.bindingVersion !== credential.machineWorkloadBindingVersion ||
      decision.bindingStatus !== 'ACTIVE' ||
      decision.workloadSpiffeId !== workload.spiffeId ||
      !decision.decisionReference ||
      !decision.scopeLevel ||
      (decision.scopeLevel === 'SYSTEM' && decision.tenantId) ||
      (decision.scopeLevel === 'TENANT' && !decision.tenantId)
    )
      throw new Error('EXECUTION_MACHINE_BINDING_STALE')
    return Object.freeze({
      subject: credential.machinePrincipalId,
      principalType: 'MACHINE',
      scopeLevel: decision.scopeLevel,
      ...(decision.tenantId ? { tenantId: decision.tenantId } : {}),
      ...(decision.orgId ? { orgId: decision.orgId } : {})
    })
  }
}
...
function isAllowedMachineType(value: unknown): boolean {
  return value === 'INTERNAL_SERVICE' || value === 'AUTOMATION_BOT'
}
