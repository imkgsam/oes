...
export class MachineWorkloadSourceCredentialVerifier {
  ...
  private async verifyUnchecked(sourceCredential: string, workload: VerifiedExecutionWorkload): Promise<TrustedExecutionContext> {
    ...
    const now = this.now()
    if (header.typ !== 'oes-machine-source+jwt' || header.alg !== 'ES256' || typeof header.kid !== 'string' || claims.iss !== this.issuer || claims.aud !== 'urn:oes:service:auth-service' || claims.profile_version !== 1 || typeof claims.jti !== 'string' || typeof claims.sub !== 'string' || typeof claims.client_id !== 'string' || claims.client_id !== workload.spiffeId || cnf?.['x5t#S256'] !== workload.certificateThumbprint || !isNumericDate(iat) || !isNumericDate(nbf) || !isNumericDate(exp) || iat > now + 60 || nbf < iat || nbf > now + 60 || exp <= iat || exp - iat > 900 || exp <= now) throw new Error(isNumericDate(exp) && exp <= now ? 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_EXPIRED' : 'EXECUTION_MACHINE_SOURCE_CREDENTIAL_INVALID')
