...
/** Obtains only Auth's configured opaque SYSTEM MACHINE source credential over mTLS. */
export class AuthNotificationMachineSourceCredentialClient {
  private client?: ClientGrpc
  private service?: MachineWorkloadSourceCredentialServiceClient

  async issue(): Promise<string> {
    const machinePrincipalId = required('AUTH_NOTIFICATION_MACHINE_PRINCIPAL_ID')
    const machineWorkloadBindingId = required('AUTH_NOTIFICATION_MACHINE_WORKLOAD_BINDING_ID')
    const machineWorkloadBindingVersion = required('AUTH_NOTIFICATION_MACHINE_WORKLOAD_BINDING_VERSION')
    ...
    if (result.tokenType !== 'Bearer' || !result.sourceCredential?.trim()) {
      throw new Error('AUTH_NOTIFICATION_MACHINE_SOURCE_CREDENTIAL_INVALID')
    }
...
function required(name: string): string {
  ...
  if (!value) throw new Error('AUTH_NOTIFICATION_MACHINE_CONFIGURATION_REQUIRED')
  return value
}
