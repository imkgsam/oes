...
export class ExternalApiKeyAuditAdapter {
  ...
  async record(input: { eventType: string; outcome: string; credentialId?: string; machineId: string; tenantId: string; operatorId?: string; traceId?: string; requestId?: string }): Promise<void> {
    await this.append({ service: 'auth-service', module: 'external-api-key', eventType: input.eventType, result: input.outcome, resourceType: 'external_api_key_credential', resourceId: input.credentialId, tenantId: input.tenantId, operatorId: input.operatorId, traceId: input.traceId, details: { machineId: input.machineId, requestId: input.requestId } })
  }
}
