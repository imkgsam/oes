...
export class ExecutionTokenGrpcController implements ExecutionTokenServiceController {
  ...
  async exchangeExecutionToken(request: ExchangeExecutionTokenRequest, _metadata?: unknown, call?: unknown): Promise<ExchangeExecutionTokenResponse> {
    const trusted = await this.context.resolve(call, { targetAudience: request.targetAudience, requestedPermissionCodes: request.requestedPermissionCodes })
    const result = await this.exchangeService.exchange({ targetAudience: request.targetAudience, requestedPermissionCodes: request.requestedPermissionCodes, ...trusted })
    return { accessToken: result.accessToken, tokenType: result.tokenType, expiresAtUnixSeconds: String(result.expiresAtUnixSeconds), expiresInSeconds: String(result.expiresInSeconds), kid: result.kid, grantedPermissionCodes: [...result.grantedPermissionCodes], grantedAudience: result.grantedAudience }
  }

  /** Returns the generated-proto JWKS response from the issuer-bound public publication service. */
  async getExecutionTokenJwks(_request: GetExecutionTokenJwksRequest): Promise<GetExecutionTokenJwksResponse> {
    const result = await this.jwksService.jwks() as any
    return { issuer: result.issuer, maxAgeSeconds: result.maxAgeSeconds, unknownKidRefreshLimit: result.unknownKidRefreshLimit, keys: result.keys, rotations: result.rotations }
  }
}
