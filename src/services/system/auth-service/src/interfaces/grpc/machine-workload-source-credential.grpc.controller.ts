...
import {
  ...
  IssueMachineWorkloadSourceCredentialRequest,
  IssueMachineWorkloadSourceCredentialResponse,
  RevokeMachineWorkloadSourceCredentialRequest,
  RevokeMachineWorkloadSourceCredentialResponse
} from '@oes/common/generated/auth_service'
import {
  IssueMachineWorkloadSourceCredentialCommand,
  RevokeMachineWorkloadSourceCredentialCommand
...
export class MachineWorkloadSourceCredentialGrpcController implements MachineWorkloadSourceCredentialServiceController {
  constructor(
    ...
    @Inject(GrpcWorkloadIdentityProvider)
  ...
  async issueMachineWorkloadSourceCredential(
    ...
    return {
      sourceCredential: result.sourceCredential,
      credentialId: credential.id,
      tokenType: 'Bearer',
      issuedAtUnixSeconds: Math.floor(credential.issuedAt.getTime() / 1_000).toString(),
      expiresAtUnixSeconds: Math.floor(credential.expiresAt.getTime() / 1_000).toString(),
      machinePrincipalId: credential.machinePrincipalId,
      machineWorkloadBindingId: credential.machineWorkloadBindingId,
      machineWorkloadBindingVersion: credential.machineWorkloadBindingVersion.toString(),
      auditCorrelationId: credential.auditId,
      supersedesCredentialId: result.supersedesCredentialId
    }
  }

  /** Revokes one credential through the normal protected management identity path. */
  @RequirePermissions({
    all: [AUTH_MANAGEMENT_PERMISSION_CODES.REVOKE_MACHINE_WORKLOAD_SOURCE_CREDENTIAL]
  })
  @UseGuards(InternalServiceGuard, AuthenticatedOperatorGuard, PermissionGuard)
  async revokeMachineWorkloadSourceCredential(
    request: RevokeMachineWorkloadSourceCredentialRequest
  ): Promise<RevokeMachineWorkloadSourceCredentialResponse> {
    if (!MACHINE_SOURCE_REVOCATION_REASONS.has(request.reasonCode ?? ''))
    ...
    const result = await this.commandBus.execute(
      new RevokeMachineWorkloadSourceCredentialCommand({
    ...
    return {
      ...
      revokedAtUnixSeconds: Math.floor(result.credential.revokedAt.getTime() / 1_000).toString(),
      alreadyRevoked: result.alreadyRevoked,
      auditCorrelationId: `machine-source-revoke:${result.credential.id}`
    }
  }
}

/** Restricts lifecycle revocation to the frozen non-enumerating management reason vocabulary. */
const MACHINE_SOURCE_REVOCATION_REASONS = new Set([
  'COMPROMISED',
  'OPERATOR_REQUEST',
  'BINDING_DISABLED'
])
