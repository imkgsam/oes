    ...
    try {
      await this.runStep(run.id, steps, TenantOnboardingStepKey.REGISTER_ORGANIZATION_PARTY, async () => {
        ...
        const result = await this.partyRegistrationPort.registerOrganizationParty({
          canonicalName: input.organizationParty.legalName,
...
function createInitialSteps(): TenantOnboardingStepRecord[] {
  return Object.values(TenantOnboardingStepKey).map((key) => ({
    key,
...
function normalizeInput(input: StartTenantOnboardingInput): StartTenantOnboardingInput {
  ...
  return {
    ...
    organizationParty: {
      ...
      registeredCountry: input.organizationParty.registeredCountry?.trim() || undefined,
      identifiers: (input.organizationParty.identifiers ?? []).map((identifier) => ({
        identifierType: requireNonBlank(identifier.identifierType, 'identifier.identifierType'),
        rawValue: identifier.rawValue?.trim() || identifier.normalizedValue?.trim(),
        normalizedValue: requireNonBlank(identifier.normalizedValue, 'identifier.normalizedValue'),
        issuerCountryOrRegion: identifier.issuerCountryOrRegion?.trim() || undefined
      }))
    },
    rootOrg: {
