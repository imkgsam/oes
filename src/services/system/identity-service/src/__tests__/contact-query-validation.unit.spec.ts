...
import { ListAccountWorkEmailAssetsQuery } from '../application/queries/contact/list-account-work-email-assets.query'

describe('contact query 校验', () => {
  it('查询工作邮箱资产 / 当 accountId 不是 UUID 时 / 应返回 VALIDATION_FAILED', async () => {
    const queryBus = {
      execute: jest.fn()
