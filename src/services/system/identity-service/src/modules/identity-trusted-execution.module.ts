...
const CALLERS = new Set(['api-gateway', 'auth-service', 'permission-service', 'hr-service', 'tenant-org-service', 'collaboration-service', 'public-entry-service'])

/** Narrows Identity's verified baseline calls to the frozen direct, OBO and pre-auth workloads. */
@Injectable()
export class IdentityFoundationTrustedExecutionGuard extends TrustedExecutionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!CALLERS.has(workload)) throw new ForbiddenException('Identity caller workload is not permitted')
    if (token?.principalType === 'HUMAN' && token.sessionTerminal !== 'WEB') throw new ForbiddenException('Identity HUMAN execution requires WEB terminal')
    if (token?.principalType === 'MACHINE' && !['auth-service', 'public-entry-service'].includes(workload)) throw new ForbiddenException('Identity SYSTEM MACHINE caller is not permitted')
    return true
  }
}
