...
describe('Meilong Runtime public surface governance', () => {
  ...
  it('keeps Product detail operation-specific while removed list and Product Category exposures stay 404', async () => {
    const controller = new PublicDataController(
      new PublicDataService(exposure, runtimeService as never)
    )
  ...
  it('retries the complete public list boundary when page governance switches mid-read', async () => {
    ...
    const controller = new PublicDataController(
      new PublicDataService(versionedExposure, versionedRuntimeService as never)
    )
  ...
  it('loads each semantic SEO collection once per locale and publication attempt', async () => {
    ...
    for (const locale of locales) {
      ...
      for (const collection of ['products', 'blog', 'news']) {
        expect(instrumented.getFullLoadCount(collection, locale)).toBeLessThanOrEqual(1)
      }
      // Blog and News categories share one physical reader but require two
      // independently filtered membership projections.
      expect(instrumented.getFullLoadCount('article-categories', locale)).toBe(2)
