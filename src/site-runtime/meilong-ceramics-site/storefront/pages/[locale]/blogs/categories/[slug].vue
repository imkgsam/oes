...
const { data: categories } = await useArticleCategories('blog', 'BLOG_CATEGORY', locale)
...
const { data: archive } = await useArticleCategoryArchive('blog', canonicalCategorySlug, page, {
  locale,
...
// categoryPath preserves the locale-qualified Storefront route for category navigation and redirects.
function categoryPath(slug: string, currentPage = 1): string {
  const base = `/${locale}/blogs/categories/${slug}`
  return currentPage > 1 ? `${base}?page=${currentPage}` : base
}

// canonicalPageUrl produces the locale-qualified canonical URL used in metadata and pagination relations.
...
// articlePath returns the locale-qualified article URL used by the category archive's structured data.
