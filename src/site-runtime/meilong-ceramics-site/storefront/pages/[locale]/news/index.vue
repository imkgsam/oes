...
const { data: categories } = await useArticleCategories('news', 'NEWS_LIST', locale)
...
const canonical = computed(
  () => routeCanonical.value ?? `${siteConfig.value?.publicBaseUrl ?? 'https://maidstonedxv.com'}/${locale}/news`
)
...
// newsArchiveOptions keeps localized SSR and continuation requests on the same complete-collection filter.
...
// loadNewsPage requests the next bounded page without crossing the localized News archive session.
...
// buildNewsIndexStructuredData describes the visible localized newsroom feed without exposing internal Runtime routes.
...
<template>
  <NewsArchive
    ...
    :date-filter-action="`/${locale}/news`"
