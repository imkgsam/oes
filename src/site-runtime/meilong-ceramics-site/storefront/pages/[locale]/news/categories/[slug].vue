...
const { data: categories } = await useArticleCategories('news', 'NEWS_CATEGORY', locale)
...
// newsArchiveOptions keeps localized category SSR and continuation reads on one complete-set date filter.
...
// loadNewsPage requests the next localized filtered category page through its operation-specific API.
...
// categoryPath preserves the locale-qualified Storefront category route for navigation and historical redirects.
function categoryPath(slug: string, currentPage = 1): string {
  const base = `/${locale}/news/categories/${slug}`
  return currentPage > 1 ? `${base}?page=${currentPage}` : base
}

// canonicalPageUrl produces the locale-qualified self-canonical and pagination relation for this category.
...
// buildCategoryStructuredData preserves global News positions across localized category pages.
    ...
    mainEntity: {
      ...
      itemListElement: items.map((item, index) => ({
        ...
        item: {
          ...
          url: new URL(`/${locale}/news/${item.slug}`, canonicalUrl).toString()
        }
      }))
    }
  }
