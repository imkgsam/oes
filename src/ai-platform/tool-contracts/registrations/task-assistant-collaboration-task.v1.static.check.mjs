...
const ORIGINAL_LEASED_PATHS = [MANIFEST_PATH, ORIGINAL_TEST_PATH].sort();
...
test('limits the registration candidate to the exact two-file implementation lease', () => {
  ...
  // The introduction commit predates the taxonomy cutover, so it legitimately
  // contains the legacy test filename. The current tree is checked separately
  // by discovery and this executable static rule.
  assert.deepEqual(changedPaths, introductionCommit ? ORIGINAL_LEASED_PATHS : LEASED_PATHS);
});
...
test('has no production source path that imports, loads, or executes the manifest', () => {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (
        [...LEASED_PATHS, ORIGINAL_TEST_PATH].includes(repositoryPath) ||
