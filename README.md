...
All local development, focused integration, and CI dependency allocation now use the same launcher;
there is no legacy/V2 mode selector.
...
pnpm runtime:plan -- --profile DEV --test-class integration --owner permission-service
```

Optional machine tunables may be copied from `.env.example` to
`~/.config/oes/local.env`. Endpoints, ports, identities, credentials, databases, buckets, and
certificates are launcher-owned and never read from a worktree dotenv file.

### Start A Development Scope

```bash
pnpm dev:system -- --task-key developer_dev
# or: pnpm dev:business -- --task-key developer_dev
# or: pnpm dev -- --task-key developer_dev
```

The command provisions or reopens the machine-shared DEV providers, snapshots selected persistent
data, builds generated/shared prerequisites, applies committed migrations and Foundation Seed,
reconciles machine workload selectors, starts the protected signer and HTTPS issuer when Auth is
selected, then publishes dynamic host-process endpoints. `Ctrl-C` stops host processes and
reconciles only exact run-owned resources; shared DEV state remains.
...
See [the executable runtime runbook](docs/runbooks/local-development-and-test-runtime.md).
