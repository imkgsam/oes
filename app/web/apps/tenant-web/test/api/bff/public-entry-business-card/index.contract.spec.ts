...
describe('tenant-web public entry business card api', () => {
  ...
  it('calls admin and self-view BFF endpoints without sending contact values as BusinessCard truth', async () => {
    ...
    } = await import('../../../../src/api/bff/public-entry-business-card/index');

    await ensurePrimaryBusinessCardApi('tenant_001', 'emp_001');
    await listBusinessCardsApi('tenant_001', {
      employeeId: 'emp_001',
      page: 2,
      pageSize: 10,
    });
    ...
    expect(get).toHaveBeenCalledWith(
      '/public-entry/tenants/tenant_001/business-cards',
      {
        params: { employeeId: 'emp_001', page: 2, pageSize: 10 },
      },
    );
    expect(get).toHaveBeenCalledWith(
      '/public-entry/tenants/tenant_001/business-cards/card_001',
    );
    expect(get).toHaveBeenCalledWith(
    ...
    expect(get).toHaveBeenCalledWith(
      '/public-entry/tenants/tenant_001/business-cards/self/preview',
    );
  });

  it('uses anonymous public API paths and maps bad responses to generic unavailable', async () => {
    const { renderPublicBusinessCardApi, resolveBusinessCardVCardUrl } =
      await import('../../../../src/api/bff/public-entry-business-card/index');
        ...
        data: {
          ...
          view: {
            ...
            contactActions: [
              ...
              {
                actionUrl:
                  'https://wrong.example/public/business-cards/other.vcf',
    ...
    await expect(
      renderPublicBusinessCardApi('card_hidden_company'),
    ).resolves.toEqual({
      state: 'AVAILABLE',
      view: {
        businessCardId: 'card_hidden_company',
    ...
    fetchMock.mockResolvedValueOnce({
      ...
      json: async () => ({
        ...
        internalReason: 'tenant mismatch',
      }),
      ok: false,
      status: 404,
    });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/public-entry/public/business-cards/card_001',
      {
