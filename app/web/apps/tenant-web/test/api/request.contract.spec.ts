...
describe('tenant-web request refresh token resolution', () => {
  ...
  it('keeps using the in-memory refresh token when no persisted override exists', async () => {
    const setRefreshToken = vi.fn()
    const { resolveRefreshTokenForRequest } = await import('../../src/api/request')
...
describe('tenant-web request auth recovery error suppression', () => {
  it('suppresses expected auth recovery errors', async () => {
    const { shouldSuppressAuthRecoveryError } = await import('../../src/api/request')
...
describe('tenant-web request local error handling suppression', () => {
  it('suppresses global error messages only when a request explicitly opts in', async () => {
    const { shouldSuppressRequestErrorMessage } = await import('../../src/api/request')
...
describe('tenant-web request validation message mapping', () => {
  it.each([
    ...
    const { resolveUserFacingErrorMessage } = await import('../../src/api/request')
...
describe('tenant-web refresh session payload extraction', () => {
  it('extracts the nested refresh session payload from the gateway success envelope', async () => {
    const { extractRefreshSessionPayload } = await import('../../src/api/request')
