...
const basePath = (tenantId: string) =>
  `/public-entry/tenants/${tenantId}/business-cards`;

// ensurePrimaryBusinessCardApi ensures one primary employee card through the admin BFF.
export function ensurePrimaryBusinessCardApi(
  tenantId: string,
  employeeId: string,
) {
...
export function listBusinessCardsApi(
  ...
  return requestClient.get<PublicEntryBusinessCardApi.ListResult>(
    basePath(tenantId),
    {
      params: query,
    },
...
export function getBusinessCardDetailApi(
  tenantId: string,
  businessCardId: string,
) {
...
export function listBusinessCardContactAssetCandidatesApi(
  tenantId: string,
  employeeId: string,
) {
...
export function enableBusinessCardApi(
  tenantId: string,
  businessCardId: string,
) {
...
export function disableBusinessCardApi(
  tenantId: string,
  businessCardId: string,
) {
...
export function bindBusinessCardPublicEntryApi(
  tenantId: string,
  businessCardId: string,
) {
  return requestClient.post<{
    publicEntryRef: PublicEntryBusinessCardApi.PublicEntryRef;
  }>(`${basePath(tenantId)}/${businessCardId}/public-entry`);
}

// getBusinessCardVisitSummaryApi reads visit summary through ShortLink.
export function getBusinessCardVisitSummaryApi(
  tenantId: string,
  businessCardId: string,
) {
...
function normalizePublicRenderResponse(
  ...
  const result =
    envelope.data ?? (payload as PublicEntryBusinessCardApi.PublicRenderResult);
  if (
    result?.state === 'PUBLIC_CARD_NOT_FOUND' ||
    result?.state === 'PUBLIC_CARD_UNAVAILABLE'
  ) {
...
function normalizePublicView(
  value: unknown,
): PublicEntryBusinessCardApi.PublicView | undefined {
  if (!isRecord(value) || !isRecord(value.person) || !isRecord(value.company))
    return undefined;
  ...
  return {
    ...
    contactActions: Array.isArray(value.contactActions)
      ? value.contactActions.flatMap((action) =>
          normalizePublicAction(action, businessCardId),
        )
      : [],
...
function normalizePublicAction(
  ...
  if (
    !PUBLIC_ACTION_TYPES.has(
      contactActionType as PublicEntryBusinessCardApi.ActionType,
    ) ||
