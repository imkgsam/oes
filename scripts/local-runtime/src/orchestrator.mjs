...
import { acquireFifoSlot, releaseFifoIdentity, withExclusiveLock } from './locks.mjs'
...
export async function startRuntime(intent, adapters = {}) {
  ...
  const transaction = { schemaVersion: 2, lifecycle: 'ALLOCATING', ...context, plan, config: redact(config), resources: [], endpoints: [] }
  ...
  try {
    ...
    return { ...published, releaseSlot, cleanup, context }
  } catch (primary) {
...
export function reconcileRuntime({ manifestPath, transactionPath, cleanupResource, releaseSlot = () => {} }) {
...
export async function withRuntime(intent, callback, adapters = {}) {
  ...
  try { return await callback(started.manifest, started.file) } catch (error) { primary = error; throw error } finally {
    try { reconcileRuntime({ manifestPath: started.file, cleanupResource: started.cleanup, releaseSlot: started.releaseSlot }) } catch (cleanupError) {
