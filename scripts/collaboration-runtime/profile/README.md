...
`OWNER_GIT_DIRECTORY` must be the `.git` directory of the owner-exclusive clone, never a shared Git common directory. The authorization root is profile-read-only; only the creating parent/Human-gate issuer writes action records. The serial-admission root contains only the global exact lock and grants no ref/worktree access. Keep sensitive-file denies and the domain allowlist unchanged. Read back the installed profile SHA and execute the full `profile-preflight` smoke before accepting it.
