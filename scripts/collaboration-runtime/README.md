...
- `src/proposal-queue.ts`: read-only FIFO derivation from exact UD native history, immutable Proposal/source/return binding, duplicate/superseded handling, single-flight admission, and on-demand bounded visibility.
- `src/retry-policy.ts`: bounded transient retry with exponential backoff/jitter, immediate permission blockers, and same-SHA one-time CI infrastructure recovery decisions.
- `src/local-main.ts`: read-only designated-checkout eligibility plus exact Human-confirmed `fetch` and `ff-only` convergence; dirty, diverged, non-main, or active-operation state is preserved.
...
node --experimental-strip-types scripts/collaboration-runtime/src/cli.ts \
  ci-recovery-decision --input ci-failure-observation.json

node --experimental-strip-types scripts/collaboration-runtime/src/cli.ts \
  local-main --binding local-main-sync-binding.json
...
Proposal queue output is always derived from the supplied exact UD native message/receipt history and is never persisted as a scheduler, inbox, registry, or second queue. A terminal Proposal receipt requires the same immutable fingerprint, source task, exact return task, terminal status, and delivery key. Transient remote reads or mutations perform at most three retries; a mutation retry first rereads live truth and reuses an already-satisfied postcondition. Local-main inspection never fetches or mutates a checkout, while sync requires a sealed Human confirmation and rechecks every guard before and after the single `ff-only` update.
