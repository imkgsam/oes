...
- `src/stage-merge.ts`: immutable Stage merge-card validation, ordered single-item admission, healthy-prefix preservation, suffix stop, and content-equivalent moving-main revision.
- `src/stage-lifecycle.ts`: cleanup-intent roster validation plus dependency-ordered, partial-retry automatic archive planning that excludes the long-lived Global UD.
...
A Stage card's `authorizationFingerprint` is passed as the existing remote driver's `mergeAuthorizationFingerprint`. `stage-merge-plan` admits no more than the first incomplete ordered item; a failed item stops the same-Stage suffix while retaining the verified prefix. `stage-merge-revision` accepts latest-main refresh only when patch, content, scope, risk, and ordered-set fingerprints remain bound to the original card. `stage-lifecycle-plan` runs after cleanup-intent inventory and returns archive actions only after Stage exit, full terminal roster, and resource cleanup are verified. It exposes only the earliest incomplete dependency tier, reuses exact archived results, retries an exact failed tier, blocks its dependants, and excludes Global UD.

The required `CI` workflow remains the legacy authoritative control until the frozen performance sample passes. Pull-request runs are grouped by PR identity and stale heads are cancelled; push and merge-group runs have unique groups. The separate `CI Optimized Shadow` workflow is non-required and mutation-free. Its PR path prepares generation/build once into a deterministic content-addressed artifact containing every required service/common `dist`, generated-contract, and Prisma-client root. It verifies the 21 identical Prisma engines once, stores one copy plus an exact target manifest, and reconstructs same-digest engine links after extraction; both unit and L2 shards verify and restore that exact artifact. L2 shards start only task-owned Postgres/NATS, migrate only the databases selected by their deterministic inventory, and always remove their containers, volumes, and networks. Its main path proves the Merge Commit's two parents, accepted PR head, base, tree, lockfile, toolchain, evidence fingerprints, and artifact digest before running fast smoke; absent equivalence runs the full fallback rather than silently trusting reuse. Evidence binds source tree/base and prepared-artifact digest while keeping paired `LEGACY_CONTROL`/`OPTIMIZED_SHADOW` workload identity equal. A missing, failed, cancelled, or unpaired shadow never grants merge authority or changes `Baseline Checks`.
