...
- `src/retry-policy.ts`: bounded transient retry with exponential backoff/jitter, immediate permission blockers, and a monotonic workflow-run/job/SHA receipt that authorizes one same-SHA CI infrastructure rerun and replays after response loss.
- `src/local-main.ts`: read-only designated-checkout eligibility plus profile-trusted Human confirmation, exact realpath/ref/SHA binding, monotonic transition recovery, and guarded `fetch`/`ff-only` convergence; dirty, diverged, non-main, symlink-drift, or active-operation state is preserved.
...
node --experimental-strip-types scripts/collaboration-runtime/src/cli.ts \
  ...
  --input ci-failure-observation.json
