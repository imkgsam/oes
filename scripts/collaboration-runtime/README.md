...
- `src/profile-preflight.ts`: mandatory two-phase production capability probes, v1 read compatibility, v2-only writing, exact draft/session/turn-bound issuer telemetry records, all-context managed/restricted telemetry hashing, zero-normal-prompt acceptance, and bounded defect/repair routing.
...
# After that target turn completes and its rollout plus exact snapshot record are issuer-sealed:
node --experimental-strip-types scripts/collaboration-runtime/src/cli.ts \
...
The required `CI` workflow remains the legacy authoritative control until the frozen performance sample passes. Pull-request runs are grouped by PR identity and stale heads are cancelled; push and merge-group runs have unique groups. The separate `CI Optimized Shadow` workflow is non-required and mutation-free. Its PR path prepares generation/build once into a deterministic content-addressed artifact containing every required service/common `dist`, generated-contract, and Prisma-client root. It verifies the 21 identical Prisma engines once, stores one copy plus an exact target manifest, and reconstructs same-digest engine links after extraction. Unit keeps two weighted lanes inside one job. L2 keeps three weighted package lanes behind one task-owned Postgres/NATS/bootstrap lifecycle and one migration pass, but serializes the Collaboration/Notification live-NATS conflict group in one lane; the runner waits for all lanes and then proves cleanup. The main path compares the final Merge Commit's exact base/head/tree against PR evidence whose accepted-result identity is the stable tree rather than the synthetic PR merge SHA; all other toolchain/workflow/inventory/artifact inputs must match, otherwise the same run executes full fallback. The performance gate accepts only a GitHub-readback sample reference beneath the installed profile's owner-read-only authorization root; direct caller JSON cannot produce `CUTOVER_READY`. Missing, failed, cancelled, unpaired, or incomplete shadow evidence never grants merge authority or changes `Baseline Checks`.
