...
const envSource = resolve(
  process.env.OES_TASK_ENV || join(root, '.tmp/oes-database-lifecycle', taskKey, 'compose.env')
)
...
export async function readInventory(text) {
  ...
  for (const key of ['workload', 'canonicalPort', 'source']) {
    ...
    if (new Set(values).size !== values.length)
      throw new Error(`TRUSTED_RUNTIME_DUPLICATE_${key.toUpperCase()}`)
  }
  return listeners
}
...
export async function generateProfile({
  ...
  const nacosPort =
    ...
    (requireInfrastructure ? resolveInfrastructurePort('nacos', '8848') : '8848')
  const postgresPort = requireInfrastructure
    ? resolveInfrastructurePort('postgres', '5432')
    : '5432'
  ...
  const notificationPayloadKey = await stableBase64Secret(
    join(stateRoot, 'secrets/notification-delivery-payload.key'),
    32
  )
  const endpoints = Object.fromEntries(
    inventory.map((entry, index) => [entry.workload, basePort + index])
  )
  const manifest = {
    version: 1,
    taskKey,
  ...
  for (const entry of inventory) {
    ...
    const env = {
      ...sourceEnvironment,
      ...(composeEnvironment[entry.workload] || {}),
      ...runtimePolicies,
      ...(runtimeSelectors[entry.workload] || {})
    }
    for (const [name, value] of Object.entries(env)) {
      if (name.endsWith('DATABASE_URL') && value)
        env[name] = rewriteDatabaseUrl(value, postgresPort)
    }
    ...
    Object.assign(env, {
      ...
      DATABASE_URL: rewriteDatabaseUrl(
        sourceEnvironment[`OES_DB_${entry.workload.replaceAll('-', '_').toUpperCase()}_URL`] ||
          env.DATABASE_URL ||
          '',
        postgresPort
      ),
    ...
    if (entry.workload === 'notification-service')
      env.NOTIFICATION_DELIVERY_PAYLOAD_KEY = notificationPayloadKey
    ...
    manifest.services.push({
      ...
      packageDirectory,
      port: endpoints[entry.workload],
      group: serviceGroup(entry.workload),
      serverName: `${entry.workload}.localhost`,
      spiffeId: env.OES_WORKLOAD_SPIFFE_ID,
      envPath,
      certPath: env.OES_GRPC_TLS_CERT_PATH,
      logPath: join(stateRoot, 'logs', `${entry.workload}.log`),
      pidPath: join(stateRoot, 'pids', `${entry.workload}.pid`)
    })
  }
  ...
  const gatewayEnvironment = {
    ...sourceEnvironment,
    ...(composeEnvironment['api-gateway'] || {}),
    ...runtimePolicies,
    ...(runtimeSelectors['api-gateway'] || {}),
    ...endpointEnvironment(endpoints)
  }
  ...
  gatewayEnvironment.NODE_OPTIONS = [gatewayEnvironment.NODE_OPTIONS, `--require=${issuerResolver}`]
    .filter(Boolean)
    .join(' ')
  ...
  manifest.gateway = {
    ...
    packageDirectory: gatewayDirectory,
    port: gatewayPort,
    envPath: gatewayEnvPath,
    certPath: gatewayEnvironment.OES_GRPC_TLS_CERT_PATH,
    logPath: join(stateRoot, 'logs/api-gateway.log'),
    pidPath: join(stateRoot, 'pids/api-gateway.pid')
  }
  manifest.issuer = {
    port: issuerPort,
...
async function runtimeSelectorEnvironment() {
  ...
  return Object.fromEntries(
    Object.entries(mappings).map(([workload, prefixes]) => {
      const selector = selectors.get(workload)
      if (!selector)
        throw new Error(
          `TRUSTED_RUNTIME_SELECTOR_MISSING_${workload.toUpperCase().replaceAll('-', '_')}`
        )
      return [
        workload,
        Object.fromEntries(
...
async function runtimePolicyEnvironment() {
  const [authSource, permission] = await Promise.all([
    ...
    readFile(
      join(root, 'scripts/local/runtime-config/permission-workload-issuance-policies.json'),
      'utf8'
    )
  ])
  ...
  const gateway = auth.find(
    (entry) => entry.spiffeId === 'spiffe://local.oes.internal/ns/oes/sa/api-gateway'
  )
  ...
  gateway.humanObo = {
    ...
    targetAudiences: gatewayHumanOboTargetAudiences
  }
  const collaborationSelector = selectorProfile.selectors.find(
    (entry) => entry.inventoryEntryKey === 'collaboration-service'
  )
  if (!collaborationSelector) throw new Error('TRUSTED_RUNTIME_SELECTOR_MISSING_COLLABORATION')
  const collaboration = auth.find(
    (entry) => entry.spiffeId === 'spiffe://local.oes.internal/ns/oes/sa/collaboration-service'
  )
...
async function startApisix(manifest) {
  ...
  if (!source.includes('api-gateway:9101'))
    throw new Error('TRUSTED_RUNTIME_APISIX_UPSTREAM_INVALID')
  await writeFile(
    runtimeConfig,
    source.replaceAll('api-gateway:9101', `host.docker.internal:${manifest.gateway.port}`),
    { mode: 0o600 }
  )
  spawnSync('docker', ['rm', '--force', container], { encoding: 'utf8', timeout: 15_000 })
  await spawnChecked('docker', [
    'run',
    '--detach',
    '--name',
    container,
    '--label',
    `oes.local.owner=${taskKey}`,
    '--label',
    'oes.local.scope=gateway-events',
    '--add-host',
    ...
    '--volume',
    `${runtimeConfig}:/usr/local/apisix/conf/apisix.yaml:ro`,
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    try {
      ...
      /* Keep polling the bounded task-owned route. */
    }
...
async function startSigner(manifest) {
  const signer = {
    pidPath: join(stateRoot, 'pids/signer.pid'),
    logPath: join(stateRoot, 'logs/signer.log')
  }
  ...
  if (await livePid(signer.pidPath)) {
    ...
    await appendEnvironment(
      manifest.services.find((service) => service.workload === 'auth-service').envPath,
      { AUTH_EXECUTION_SIGNER_SOCKET_PATH: socket, AUTH_EXECUTION_KMS_KEY_REF: keyReference }
    )
    return
  }
  ...
  spawnSync('docker', ['stop', `oes_${taskKey}-execution-token-signer-1`], {
    encoding: 'utf8',
    timeout: 15_000
  })
  const child = spawn(
    join(root, 'docker/grpc-trust/execution-token-signer/local/softhsm2/run-host.sh'),
    [],
    {
      cwd: root,
      env: {
        ...process.env,
        EXECUTION_SIGNER_RUNTIME_MODE: '1',
        EXECUTION_SIGNER_HOST_WORK_DIR: work,
        EXECUTION_SIGNER_KEEP_HOST_WORK_DIR: '1',
        EXECUTION_SIGNER_READY_PATH: ready,
        AUTH_EXECUTION_SIGNER_SOCKET_PATH: socket,
        AUTH_EXECUTION_PKCS11_MODULE: module
      },
      detached: true,
      stdio: ['ignore', await openAppend(signer.logPath), await openAppend(signer.logPath)]
    }
  )
  child.unref()
  await writeFile(signer.pidPath, `${child.pid}\n`, { mode: 0o600 })
  for (let attempt = 0; attempt < 60; attempt += 1) {
    try {
      const keyReference = (await readFile(ready, 'utf8')).trim()
      await appendEnvironment(
...
async function startDockerSigner(manifest, work, ready, socket) {
  ...
  const existing = spawnSync(
    'docker',
    ['inspect', '-f', '{{.State.Running}}', `oes_${taskKey}-execution-token-signer-1`],
  ...
  if (existing.status === 0 && existing.stdout.trim() === 'true') {
    try {
      ...
      /* Recreate an unhealthy or incomplete signer below. */
    }
  }
  await spawnChecked('docker', [
    'compose',
...
function hostSoftHsmModule() {
  for (const path of [
    join(stateRoot, 'softhsm/softhsm/2.7.0/lib/softhsm/libsofthsm2.so'),
    ...
    try {
      ...
      /* Docker signer is the bounded fallback. */
    }
  }
  return null
}
...
async function ensureTaskLocalSoftHsm() {
  ...
  if (existing) {
    ...
    if (bytes.includes(Buffer.from('@@HOMEBREW_PREFIX@@'))) {
      await spawnChecked('install_name_tool', [
        '-change',
  ...
  if (
    ...
    spawnSync('brew', ['--version'], { timeout: 5_000 }).status !== 0
  )
    return null
...
async function startService(service) {
  ...
  try {
    await stat(join(service.packageDirectory, 'dist/main.js'))
  } catch {
    await spawnChecked('pnpm', ['--filter', service.packageName, 'build'])
  }
  const environment = parseEnv(await readFile(service.envPath, 'utf8'))
  const out = await openAppend(service.logPath)
  const child = spawn('pnpm', ['--filter', service.packageName, 'start'], {
    cwd: root,
...
async function waitReady(services, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    if (
      (await Promise.all(services.map((service) => canConnect('127.0.0.1', service.port)))).every(
        Boolean
      )
    )
      return
    if (
      (await Promise.all(services.map((service) => livePid(service.pidPath)))).some((pid) => !pid)
    )
      throw new Error('TRUSTED_RUNTIME_PROCESS_EXITED')
    await new Promise((resolvePromise) => setTimeout(resolvePromise, 500))
  }
  throw new Error(
    `TRUSTED_RUNTIME_READINESS_TIMEOUT_${services.map((service) => service.workload).join('_')}`
  )
}
...
async function status(requireReady = false) {
  ...
  for (const service of [...manifest.services, ...(manifest.gateway ? [manifest.gateway] : [])]) {
    ...
    process.stdout.write(
      `${service.workload} pid=${pid || 'DOWN'} port=${service.port} reachable=${reachable}\n`
    )
    failed ||= !pid || !reachable
  }
  if (manifest.apisix) {
    const running =
      spawnSync('docker', ['inspect', '-f', '{{.State.Running}}', manifest.apisix.container], {
        encoding: 'utf8',
        timeout: 5_000
      }).stdout.trim() === 'true'
    const reachable = await canConnect('127.0.0.1', manifest.apisix.port)
    process.stdout.write(
      `apisix container=${running ? 'UP' : 'DOWN'} port=${manifest.apisix.port} reachable=${reachable}\n`
    )
  ...
  if (manifest.issuer) {
    ...
    process.stdout.write(
      `issuer pid=${pid || 'DOWN'} port=${manifest.issuer.port} reachable=${reachable}\n`
    )
    failed ||= !pid || !reachable
  }
...
async function down() {
  ...
  if (!hostSoftHsmModule())
    await spawnChecked('docker', [
      ...
      join(root, 'docker-compose.yml'),
      '-f',
      join(stateRoot, 'signer.compose.yml'),
      'stop',
      'execution-token-signer'
    ])
  if (manifest.apisix)
    spawnSync('docker', ['stop', manifest.apisix.container], { encoding: 'utf8', timeout: 15_000 })
}
...
function serviceGroup(workload) {
  ...
  if (['asset-service', 'item-master-service', 'terminal-device-service'].includes(workload))
    return 3
  if (
    [
      'crm-service',
...
function renderComposeEnvironment() {
  ...
  return Object.fromEntries(
    Object.entries(services).map(([name, service]) => [name, service.environment || {}])
  )
}
...
async function stableBase64Secret(path, byteLength) {
  try {
    ...
    /* Create the absent task-owned secret below. */
  }
...
async function spawnChecked(file, args, extraEnv) {
  await new Promise((resolvePromise, reject) => {
    ...
    child.once('exit', (code) =>
      code === 0 ? resolvePromise() : reject(new Error(`COMMAND_FAILED_${code}`))
    )
...
async function openAppend(path) {
  ...
  return (await import('node:fs')).openSync(path, 'a', 0o600)
}

if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (command === 'check') {
    const manifest = await generateProfile()
    for (const service of manifest.services) {
      if ((await stat(service.envPath)).mode % 0o1000 !== 0o600)
