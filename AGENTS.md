...
SL在启动FL前必须区分独立交付物与同一feature的实现slice：可独立candidate、RI、PR和安全合入`main`的结果拆为平级FL；必须共同原子验收的slice保持一个FL并由FL创建有界IT。执行中发现feature已分裂为多个独立交付物时返回`FEATURE_REPLAN_REQUIRED`给exact SL；SL在已确认Stage scope、protected scope、execution capabilities、delegation ceiling与WIP内自动验证、冻结提取范围并创建sibling FL，原子feature则自动继续原FL与bounded IT，不新增Human gate。只有真实扩围、越过ceiling、owner/write冲突或阻塞性业务取舍才询问Human。parent派发child后进入`WAITING_ON_CHILD`并结束当前turn，由exact typed result唤醒，禁止持续poll或占用执行槽等待。
...
Human确认delivery scope时同时授权该scope所需的最小充分运行能力。创建任何Direct、SL、FL、IT或RI前，exact creating owner必须先把planned child identity、role/title、host/launcher、worktree、capability subset与profile fingerprint绑定到支持原子profile注入的启动路径；启动路径不能证明注入能力时不得创建task，也不得以Full Access作为普通回退。task创建后仍须由该target task从自己的当前session/turn读回实际profile并完成真实smoke；prompt、静态profile/report、其他task telemetry或父task声明均不能替代target-session evidence。profile按owner绑定exact worktree、解析后的shared Git metadata、task temp/cache、repository-declared local service/container/test database、localhost和approved network，并保护secret、生产/共享数据、host/system privilege及跨owner资源。`approval_policy=on-request`与`approvals_reviewer=auto_review`接管残余低风险platform approval；正常Direct/UD/SL/FL/IT/RI在confirmed scope内的用户权限弹窗目标值为零。

target task在handoff接受前必须对自身当前session的effective capabilities执行真实smoke，包括owner file write、Git switch/add/commit、标准build/test、task-owned service/database、localhost/network和evidence root；creating owner复核exact target-session profile fingerprint、literal smoke与approval telemetry后才转移ownership。已声明能力仍产生用户approval时属于`EXECUTION_PROFILE_DEFECT`：保持owner、candidate、state和原authorization，由creating parent或current owner自动修复、重建或迁移execution profile/host并幂等恢复，不形成Human gate。只有scope/protected scope扩大、生产或共享资源、新secret或付费外部系统、host/system privilege、cross-owner/destructive operation以及既有main merge和cleanup gate才合并请求一次Human确认。child只从Human-confirmed root topology policy为该role绑定的delegation/WIP ceiling取得完成assignment所需的更窄能力。
...
- `NEW_DESIGN`的design PR合入`main`且exact main CI通过后，UD必须在同一task主动展示动态执行建议；`NO_EXECUTION`为建议结论，Human选择暂不执行后进入`EXECUTION_DEFERRED`。`EXISTING_DELIVERY_DESIGN_GAP`将resolution返回exact original SL/FL并在guard通过时自动恢复affected lane；`ACTIVE_ACTIVATION_DESIGN_REVISION`只恢复exact原activation owner与未接受pending child，更新old truth到new canonical merge，不再发`ACTIVATION_DECISION_READY`。两类continuation都不得静默创建replacement owner。UD不得把implement发给请求来源、Design Owner或祖先task。
- `NEW_DESIGN`只有在Human确认暂不执行，或Direct/FL/SL完成两阶段handoff后，UD才进入自身cleanup-ready；`EXISTING_DELIVERY_DESIGN_GAP`在exact original owner确认resume binding或进入replan后使UD cleanup-ready；`ACTIVE_ACTIVATION_DESIGN_REVISION`在原pending child完成handoff，或continuation guard失败进入一次`ACTIVATION_REPLAN_REQUIRED`后使UD cleanup-ready。UD不接管或清理其他owner资源；editorial入口在main CI和exact source notice后直接进入UD cleanup-ready。
